...
import { AppWindow, Clapperboard, Code2, Download, FolderOpen, FolderPlus, Github, Plus, RefreshCw } from "lucide-react";
...
export default function Workspace() {
  ...
  async function updateApp(packageName: string) {
    setBusy(packageName); setError("");
    try {
      const response = await fetch(`${apiBase}/v1/apps/${encodeURIComponent(packageName)}/update`, { method: "POST" });
      if (!response.ok) throw new Error(await responseMessage(response));
      await loadWorkspace();
    } catch (cause) { setError(messageOf(cause)); } finally { setBusy(""); }
  }
  ...
  return <main className="flex h-screen min-w-[1024px] flex-col overflow-hidden bg-background">
    ...
    <div className="relative grid min-h-0 flex-1 overflow-hidden [grid-template-columns:minmax(0,1fr)_var(--side-panel-width)]" ref={layoutRef} style={{ "--side-panel-width": `${panelWidth}px` } as CSSProperties}>
      <section className="min-h-0 overflow-y-auto bg-muted/30 p-8"><div className="mx-auto max-w-6xl">
        {!online ? <ServiceGuide /> : outdated ? <ServiceGuide error={error} installedVersion={serviceVersion} onUpdate={updateService} updating={updatingService} /> : tab === "projects" ? <Projects apps={apps} name={name} onAppChange={setAppID} onNameChange={setName} onSubmit={createProject} projects={projects} selectedApp={appID} /> : <Apps installations={installations} busy={busy} error={error} onInstall={installApp} onRepositoryChange={setRepository} onUpdate={updateApp} repository={repository} />}
      </div></section>
