...
export default function Workspace() {
  ...
  return <main className="min-h-screen bg-background"><header className="flex h-12 items-center justify-between border-b bg-card px-4"><div className="flex items-center gap-2"><Clapperboard className="size-4" /><strong className="text-sm tracking-tight">RECUT</strong><Badge>LOCAL</Badge></div><div className="flex items-center gap-2 font-mono text-[10px] text-muted-foreground"><span className={online ? "size-1.5 rounded-full bg-emerald-500" : "size-1.5 rounded-full bg-amber-500"} />{online ? "DAEMON CONNECTED" : "DAEMON OFFLINE"}</div></header>
    <div className="grid min-h-[calc(100vh-3rem)] lg:grid-cols-[minmax(0,1fr)_380px]"><section className="p-5 sm:p-8"><div className="mb-8 flex items-end justify-between border-b pb-5"><div><p className="mb-2 font-mono text-[10px] text-muted-foreground">WORKSPACE / PROJECTS</p><h1 className="text-2xl font-semibold tracking-tight">项目工作区</h1></div><Badge>{projects.length} PROJECTS</Badge></div>
      ...
      <div className="grid gap-3">{projects.length === 0 ? <Card><CardContent className="flex min-h-48 flex-col items-center justify-center gap-3 text-center"><FolderOpen className="size-5 text-muted-foreground" /><div><p className="text-sm font-medium">尚无项目</p><p className="mt-1 text-xs text-muted-foreground">启动本地服务后，创建第一个本地项目。</p></div></CardContent></Card> : projects.map((project) => <Card className={projectID === project.id ? "border-foreground/40 bg-muted/30" : "transition-colors hover:bg-muted/30"} key={project.id}><button className="w-full text-left" onClick={() => setProjectID(project.id)} type="button"><CardContent className="flex items-center justify-between gap-4 py-4"><div className="min-w-0"><p className="truncate text-sm font-medium">{project.name}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">{project.id}</p></div><Badge>{project.appId}</Badge></CardContent></button></Card>)}</div>
