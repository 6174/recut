...
成员清单
page.tsx: 工作区级素材库系统应用；左侧按类型浏览、预览媒体资产，并通过“创建”菜单选择图片、视频或音频；提交后立即显示并轮询生成中任务卡片，完成时刷新真实资产；弹框以紧凑下拉框选择已连接模型，通过独立的缩略图选择弹框按需选择或上传参考图，并在主窗展示已选缩略图和移除操作；详情可将已保存的提示词、模型和引用回填以再次生成，右侧承载标准 Agent 会话。
asset-preview.tsx: 素材预览详情弹框；展示生成提示词、参考素材缩略图和再次生成入口。
