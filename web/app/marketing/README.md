...
[locale]/apps/: 逐语言公开应用市场；`page.tsx` 是 `/apps`（与 `/zh/apps`）目录，`[appID]/page.tsx` 是每个 App 的 SEO 落地页，`generateStaticParams` 返回全部 appID 与 locale 组合，由 `lib/marketing-apps` 静态数据逐语言生成并挂载逐 App 元数据与 SoftwareApplication/Breadcrumb JSON-LD；与工作台 `lib/app-catalog` 解耦，仅 `id` 保持一致以打通深链。
