...
page.tsx: 固定桌面双栏工作台；`/` Studio、`/projects`、`/media`、`/apps` 共同复用 Header、Agent 面板和各自可刷新状态的内容区。Studio 是默认创作入口，提供创作意图输入、最近项目、最近 Assets 外显和已安装 App 的快速进入；Projects 独立承载新建项目与完整项目列表；Assets 是原生媒体库；Apps 管理已安装扩展与可添加目录。所有 Studio/Projects/Apps 会话使用隐藏 general scope，Assets 使用自己的隐藏 media scope，项目详情和独立 App 则传入专属 scope，三类历史彼此隔离。新建项目使用受控 App 选择面板，只展示项目型 App、预览用途和身份、直达语义详情；顶部提供“新建应用”AI Prompt 与“从 Git 安装”入口，后者把 GitHub 仓库交由 service 校验 manifest 并在成功后刷新目录；Git App 目录读取会检测远端更新，详情和卡片可升级单项，存在可安全更新项时顶部才显示一键更新；素材库不进入 App Catalog、安装管理或 iframe；任何条目均可离线进入详情，service 仅提供安装状态、安装和 Git 更新；其余依赖 service 的工作台功能在 cloud mode 提供本地安装和已有远程 service 的连接入口，而 local mode 已与 service 同进程，不显示安装引导或独立 UI/service 版本失配；发现 service 更新时继续保留当前核心入口，由 Header 右侧的醒目操作入口触发升级；两种 scope 的新会话均显示非空 onboarding；Header 右侧复用 service 状态与全局设置操作。
