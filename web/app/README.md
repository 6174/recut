...
projects/[id]/project-detail-client.tsx: 固定桌面项目详情客户端容器；Header 展示当前加载的 App 版本与项目身份，左侧全高 iframe 承载 App UI，右侧为独立滚动且可拖拽调宽、持久化宽度的 Agent 对话栏，并转发项目事件与 iframe 请求；App API 的结构化失败原因必须透传到 iframe。
