...
standalone-app-client.tsx: 独立 App 宿主；从 `workspace-store` 共享读取 hidden workspace scope 与 App，从 `media-configuration-store` 读取已连接 Provider 的媒体模型；转发 iframe operation、Agent、媒体生成与`media.pick`请求及项目事件（含 shell job 实时日志）；直接生成被绑定到当前 scope，设置可由 App 定位到 AI 服务商分类，`agent.compose` 经全局 `lib/agent-panel-context` 只回填 Agent 输入草稿（不再提供 `agent.send` 直发）；Agent 面板由根布局全局挂载为单一全局会话，本页只声明素材上传上下文。
