...
import { createRecutWorldsClient, worldOrigin, worldScenarioDefaults, worldScenarios, worldTypes, type WorldKind, type WorldScenario } from "@/lib/recut-worlds-client";
...
export function WorldsClient() {
  ...
  // 平台世界是内容不是账户资产：置顶分段、按目录 order 稳定排序（不随
  // updated_at 漂移）；其余保持现有"最近更新优先"行为不变。
  const platformWorlds = useMemo(
    () => visible.filter((world) => worldOrigin(world) === "platform").sort((a, b) => (a.originMeta?.catalogOrder ?? 0) - (b.originMeta?.catalogOrder ?? 0)),
    [visible],
  );
  ...
  return <><div className="mb-7 flex items-end justify-between"><div><p className="mb-2 flex items-center gap-2 font-mono text-[10px] font-semibold tracking-[0.16em] text-primary"><span className="size-1.5 rounded-full bg-primary" />DESKTOP</p><h1 className="text-3xl font-semibold tracking-tight">{t("worlds.list.title")}</h1><p className="mt-2 text-sm text-muted-foreground">{t("worlds.list.desc")}</p></div><Badge className="border-primary/25 bg-accent text-accent-foreground">{state === "loading" ? t("worlds.list.loading") : state === "failed" ? t("worlds.list.failed") : interpolate(t("worlds.list.count"), { count: page.length })}</Badge></div>
    ...
    {state === "loading" ? <div className="grid grid-cols-[repeat(auto-fill,minmax(15rem,1fr))] gap-4">{Array.from({ length: 6 }, (_, index) => <div className="h-56 animate-pulse rounded-lg border bg-card" key={index} />)}</div>
      ...
      : platformWorlds.length || myWorlds.length ? <>
          {platformWorlds.length > 0 && <section className="mb-7"><h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-muted-foreground">{t("worlds.list.section.platform")}<Badge className="border-primary/25 bg-primary/10 text-primary">{t("worlds.badge.platform")}</Badge></h2><div className="grid grid-cols-[repeat(auto-fill,minmax(15rem,1fr))] gap-4">{platformWorlds.map((world) => <WorldCard apiBase={apiBase} key={world.id} world={world} />)}</div></section>}
          <section><h2 className="mb-3 text-sm font-semibold text-muted-foreground">{t("worlds.list.section.mine")}</h2>{myWorlds.length ? <div className="grid grid-cols-[repeat(auto-fill,minmax(15rem,1fr))] gap-4">{myWorlds.map((world) => <WorldCard apiBase={apiBase} key={world.id} world={world} />)}</div> : <p className="rounded-lg border border-dashed p-6 text-center text-xs text-muted-foreground">{t("worlds.list.section.mine.empty")}</p>}</section>
        </>
...
function CreateWorldDialog({ apiBase, onClose, onCreated }: { apiBase: string; onClose: () => void; onCreated: () => void }) {
  ...
  const [scenario, setScenario] = useState<WorldScenario>(worldScenarioDefaults["character_ip"]);
