...
export function WorldsClient() {
  ...
  return <><div className="mb-7 flex items-end justify-between"><div><p className="mb-2 flex items-center gap-2 font-mono text-[10px] font-semibold tracking-[0.16em] text-primary"><span className="size-1.5 rounded-full bg-primary" />DESKTOP</p><h1 className="text-3xl font-semibold tracking-tight">My Worlds</h1><p className="mt-2 text-sm text-muted-foreground">持续存在的创作上下文：角色、故事、风格与规则。所有实体只属于它们的世界。</p></div><Badge className="border-primary/25 bg-accent text-accent-foreground">{state === "loading" ? "读取中…" : state === "failed" ? "读取失败" : `${page.length} 个世界`}</Badge></div>
    <div className="mb-5 flex items-end gap-3"><label className="relative block flex-1"><span className="sr-only">搜索世界</span><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input className="h-9 bg-background pl-8 text-xs" onChange={(event) => setQuery(event.target.value)} placeholder="搜索名称或描述…" type="search" value={query} /></label><div className="w-36"><CustomSelect id="world-type-filter" label="类型筛选" onChange={(value) => setFilter(value as WorldKind | "")} options={[{ label: "全部类型", value: "" }, ...worldTypes().map((kind) => ({ label: worldKindLabels[kind], value: kind }))]} value={filter} /></div><Button className="h-9 shrink-0" onClick={() => setCreating(true)} type="button"><Plus className="size-3.5" />新建 World</Button></div>
