...
- 媒体（T8）：canvas-media.ts（source/purpose 辅助）+ canvas-media-dialogs.tsx（独立素材来源浮层 / 图片节点双击的全局素材弹框 MediaAssetPickerDialog / 预览，无目标实体通道）+ MediaBlock（媒体元素渲染）+ 拖放矩阵（文件→卡=media 属性（attachMediaAttr，无 A 线）；文件→空白=独立元素；媒体卡→卡=media 属性换挂）。属性详情面板里凡是能看到素材图的地方单击即打开素材详情弹框（AssetPreviewDialog）：media-editor 预览区图片单击、实体 media 属性字段（AssetFieldRow）；图片节点双击 = 全局素材选择弹框（AssetReferenceDialog）换图。
...
- 导入世界画布归一（canvas-store.canonicalizeCanvasElements）：bundle 里实体元素沿用源世界 id（`shape:<sourceId>`，服务端只给 refId 做世界命名空间），load 时统一改写成 `shape:<entityId>` 并重映射箭头 `props.fromElementId/toElementId`、同一实体重复投影保留 updatedAt 最新一条；否则拖拽会新建重复投影、刷新后位置/尺寸回退。归一有变化时立即落库（一次性迁移；aiLocked 期跳过后下次加载再试）。
