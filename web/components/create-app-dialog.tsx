...
export function CreateAppDialog() {
  ...
  return <><Button onClick={() => setOpen(true)} type="button"><Plus className="size-4" />新建应用</Button>{open && <div aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-foreground/30 p-6 backdrop-blur-[1px]" onMouseDown={() => setOpen(false)} role="dialog" aria-labelledby="create-app-title"><section className="w-full max-w-2xl rounded-sm border bg-card shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><header className="flex items-start justify-between gap-4 border-b px-5 py-4"><div><p className="font-mono text-[10px] font-semibold tracking-[0.16em] text-primary">CREATE APP</p><h2 className="mt-1 text-base font-semibold" id="create-app-title">让 AI 创建 Recut 应用</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">复制 Prompt 给 Codex、Claude Code 或 OpenCode；它会在你的应用目录创建完整 App 包。</p></div><button aria-label="关闭新建应用" className="grid size-8 place-items-center rounded-xs text-muted-foreground hover:bg-muted" onClick={() => setOpen(false)} type="button"><X className="size-4" /></button></header><div className="p-5"><label className="mb-2 block text-xs font-medium" htmlFor="create-app-prompt">AI 创建 Prompt</label><textarea className="h-72 w-full resize-none rounded-sm border bg-muted/30 p-3 font-mono text-xs leading-5 outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" id="create-app-prompt" readOnly value={prompt} /><a className="mt-3 inline-flex items-center gap-1.5 text-xs text-primary hover:underline" href="/app-standard.md" rel="noreferrer" target="_blank">查看 Recut 应用标准<ExternalLink className="size-3.5" /></a></div><footer className="flex items-center justify-end gap-2 border-t px-5 py-3"><Button onClick={() => setOpen(false)} type="button" variant="ghost">取消</Button><Button onClick={() => void copyPrompt()} type="button"><Copy className="size-4" />{copied ? "已复制" : "复制 Prompt"}</Button></footer></section></div>}</>;
}
