...
import { entityKindLabel, type EntityKind, type WorldEntitySummary } from "@/lib/recut-worlds-client";
...
export function WorldEntityPicker({ apiBase, open, worldId, onClose, onPick, kinds }: { apiBase: string; open: boolean; worldId: string; onClose: () => void; onPick: (pick: WorldEntityPick) => void; kinds?: EntityKind[] }) {
  ...
  useEffect(() => { if (open && worldId) { void loadEntities(apiBase, worldId, { limit: 50 }); } }, [apiBase, loadEntities, open, worldId]);
  const visible = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    return (entities ?? []).filter((entity) => (!kinds?.length || kinds.includes(entity.typeId)) && (!normalized || `${entity.name} ${entity.intro}`.toLowerCase().includes(normalized)));
