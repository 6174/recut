...
project-agent-panel.tsx: 项目与素材库共用的固定高度 Agent 对话侧栏；首页无项目时以 `scope=general` 自动进入隐藏 general scope，空态允许直接输入并在首条消息发送时创建默认 Codex 会话，支持 @ 引用项目资源或素材库资源、粘贴 `<media>` 上下文自动转引用、图片/视频/音频上传及预览 token；创建、详情同步与失败均显式呈现，发送失败保留输入并显示服务端原始原因，同步失败可直接重试，工具卡严格分离调用参数、执行结果/错误与成本信息，并提供完整查看和复制；为内部媒体卡、详情和引用选择器接入共享 Asset SSE，Agent 会话本身仍使用自己的事件流并在读取到失效 session 时自动收敛为空态，新建或点击输入框设置入口均可配置 Codex 模型和推理强度。
