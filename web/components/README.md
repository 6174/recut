...
project-agent-panel.tsx: 项目与素材库共用的固定高度 Agent 对话侧栏；首页无项目时以 `scope=general` 自动进入隐藏 general scope，空态允许直接输入并在首条消息发送时创建默认 Codex 会话，支持 @ 引用项目资源或素材库资源、粘贴 `<media>` 上下文自动转引用、图片/视频/音频上传及预览 token；面板加载即查询 Agent CLI 可用性：Codex 缺失时整块右侧面板切换为中性的三步安装、登录、验证和刷新流程，已有 Codex 但最近一次启动失败时切换为原始错误与可复制的 Codex 排障任务；已恢复可用的 CLI 不会被历史“未安装”失败事件再次阻断。恢复模式不显示会话、历史或输入框，重新检查通过后才恢复对话。Agent SSE 在 queued、running 与终态间即时同步 User Turn，避免已开始的消息仍显示待发送；历史失败以中性信息条保留。User 气泡 hover 展示发送时间，成功回复的操作行展示完成时间与总耗时。重新检查会禁用并旋转按钮、显示检查中状态；安装与诊断的复制操作兼容不安全 HTTP origin，成功或权限拒绝均会显示可见反馈；工具卡严格分离调用参数、执行结果/错误与成本信息，并提供完整查看和复制；为内部媒体卡、详情和引用选择器接入共享 Asset SSE，Agent 会话本身仍使用自己的事件流并在读取到失效 session 时自动收敛为空态，新建或点击输入框设置入口均可配置 Codex 模型和推理强度。
