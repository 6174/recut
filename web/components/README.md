...
agent-panel-host.tsx: 根布局唯一挂载的全局工作台壳；工作台时顶部固定 64px Header 横贯全宽，其下 Body 左侧单一全局 Agent 对话栏、右侧内容区与拖动手柄共同读取 `--side-panel-width`，拖拽中同步响应且跨路由持久化；壳自身以固定 Header 高度定位左栏，页面只通过 `lib/agent-panel-context` 声明 projectID/草稿/当前页面上下文，路由切换不重建面板、不重连 SSE 也不切换会话；`/marketing`、Docs、Blog 与 `localhost` / `recut.video` / `www.recut.video` 官网，都在 Host 已确认前先保持无面板状态，随后切换到无固定高度、可纵向滚动的普通文档流，且绝不挂载 Agent 面板、拖拽手柄或 App SSE，释放整页宽度给产品介绍；Host 语义优先于构建模式，故误以 local mode 启动的开发服务器也不会在 `localhost` 渲染 Chat；cloud mode 首次离线 Landing Page 同样无面板，连接阶段原样传给左栏，避免把连接中误报为不可用。
marketing-site.tsx: 官网共享展示层；提供按 Hero→核心应用→创作底座→团队主张→文章→CTA 编排的 Landing、Docs、Blog、Header/Footer 与跨域「打开工作台」链接；线上指向 `app.recut.video`，`localhost` 自动改为同端口 `app.localhost`，不读取 service 或工作台状态。
