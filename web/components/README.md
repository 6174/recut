...
agent-panel-host.tsx: 根布局唯一挂载的全局工作台壳；顶部页面 Header 横贯全宽，其下 Body 左侧单一全局 Agent 对话栏、右侧内容区与拖动手柄共同读取 `--side-panel-width`，拖拽中同步响应且跨路由持久化，页面的 projectID/Header 高度/草稿/当前页面上下文由 `lib/agent-panel-context` 声明，路由切换不重建面板、不重连 SSE 也不切换会话。
