...
project-agent-panel.tsx: 项目与素材库共用的固定高度 Agent 对话侧栏；首页无项目时以 `scope=general` 自动进入隐藏 general scope，支持 @ 引用项目资源或素材库资源、粘贴 `<media>` 上下文自动转引用、图片/视频/音频上传及预览 token；创建、详情同步与失败均显式呈现，同步失败可直接重试，输入仅在会话详情就绪后启用；工具卡严格分离调用参数、执行结果/错误与成本信息，并提供完整查看和复制；为内部媒体卡、详情和引用选择器接入共享 Asset SSE，Agent 会话本身仍使用自己的事件流并在读取到失效 session 时自动收敛为空态，新建或点击输入框设置入口均可配置 Codex 模型和推理强度。
...
service-control.tsx: Header 内的 service 控制入口；通过 Zustand 初始化并刷新唯一 endpoint 的全局状态，展示 health 提供的进程启动时间，并在启动时间变化后确认升级或 launchd 重启完成；本地已安装 daemon 才允许网页执行这些操作，远程 service 只展示连接状态。
