/*
 * [INPUT]: 依赖 service-store 的 endpoint/连接阶段、agent-panel-context 的全局 projectID/草稿、固定 64px 工作台 Header、useResizableSidePanel 与 ProjectAgentPanel
 * [OUTPUT]: 对外提供根布局挂载的全局工作台壳：顶部由页面渲染的 64px Header 横贯全宽，其下 Body 左侧为全局 Agent 对话、右侧为页面内容；左右两栏与拖动手柄共同读取 `--side-panel-width`，单实例侧栏从固定 Header 下沿起向下铺满并持久化宽度，只经 context 声明素材上下文与草稿
