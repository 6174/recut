 ...
 * [OUTPUT]: 对外提供根布局挂载的全局工作台壳：顶部由页面渲染的 Header 横贯全宽，其下 Body 左侧为页面内容、右侧为全局 Agent 侧栏；左右两栏与拖动手柄共同读取 `--side-panel-width`，单实例侧栏从 headerHeight 起向下铺满并持久化宽度，只经 context 声明素材上下文与草稿
...
export function AgentPanelHost({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <div className="relative flex min-h-screen min-w-0 flex-col overflow-hidden bg-background md:h-screen" ref={layoutRef} style={{ "--side-panel-width": `${panelWidth}px` } as CSSProperties}>
      ...
      <aside className="absolute bottom-0 right-0 z-0 hidden md:block" style={{ top: headerHeight, width: "var(--side-panel-width)" }}>
