...
import { entityKindLabels, worldKindLabels, type WorldSummary } from "@/lib/recut-worlds-client";
...
export function WorldCard({ apiBase, world }: { apiBase: string; world: WorldSummary }) {
  ...
  return <Link className="group min-w-0" href={`/worlds/${encodeURIComponent(world.id)}`}><Card className="flex h-full min-h-56 min-w-0 flex-col overflow-hidden border-transparent bg-card p-0 shadow-sm transition-all group-hover:-translate-y-1 group-hover:border-primary/35 group-hover:shadow-[var(--shadow-overlay)]">{cover ? <img alt={`${world.name} 封面`} className="h-24 w-full border-b object-cover" src={`${apiBase}/v1/media/assets/${encodeURIComponent(cover)}/content`} /> : <div className="grid h-24 w-full place-items-center border-b bg-muted text-muted-foreground"><Globe2 className="size-6" /></div>}<div className="flex min-w-0 flex-1 flex-col p-4"><div className="flex items-start justify-between gap-3"><p className="truncate text-base font-semibold">{world.name}</p><span className="rounded-xs border border-primary/20 bg-accent/60 px-1.5 py-0.5 text-[10px] text-accent-foreground">{worldKindLabels[world.type] ?? world.type}</span></div>{world.description ? <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{world.description}</p> : <p className="mt-1 text-xs text-muted-foreground">暂无描述</p>}<div className="mt-auto pt-3"><div className="flex flex-wrap items-center gap-1.5">{counts.length ? counts.map(([kind, count]) => <span className="rounded-sm border bg-secondary px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground" key={kind}>{entityKindLabels[kind as keyof typeof entityKindLabels] ?? kind} {count}</span>) : <span className="text-[10px] text-muted-foreground">还没有实体</span>}</div><div className="mt-3 flex items-center justify-between"><span className="text-[10px] text-muted-foreground">{world.updatedAt ? new Date(world.updatedAt).toLocaleString("zh-CN") : ""}</span><span className="inline-flex items-center gap-1 text-xs font-medium text-primary">进入世界<ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></div></div></div></Card></Link>;
}
