/*
 * [INPUT]: 依赖 React 生命周期与 GSAP 时间线，读取 locale 决定示例文案。
 * [OUTPUT]: 对外提供 MarketingEditorDemo；以可暂停、可循环的抽象工作台演示 Agent Chat 与真实 Editor 的素材、预览、属性、时间线结构。
  ...
  return (
    <div className="mx-auto mt-14 max-w-6xl sm:mt-20">
      <div className="marketing-editor-demo-stage">
        <div className="marketing-editor-demo-canvas w-[960px] overflow-hidden rounded-2xl border border-white/15 bg-[oklch(0.145_0.012_150)] shadow-[0_28px_110px_oklch(0_0_0_/_0.55)] lg:w-full" ref={rootRef}>
      ...
      <div className="grid grid-cols-[15.5rem_minmax(0,1fr)]">
        <aside className="flex min-h-[18rem] flex-col border-b-0 border-r border-white/10 bg-[oklch(0.13_0.01_150)] p-4">
          ...
          <div className="mt-5 flex-1 space-y-3 text-[11px] leading-5"><div className="rounded-lg border border-white/10 bg-white/[0.035] p-3 text-white/65">{zh ? "用这一组素材做一个 45 秒的产品故事，保留我对节奏和镜头的最终控制。" : "Turn this footage into a 45-second product story. Keep final control of pacing and shots with me."}</div><div className="ml-auto rounded-lg bg-primary p-3 text-primary-foreground">{zh ? "我会先整理素材和剪辑建议；每个结果都在时间线上，随时可以编辑或撤销。" : "I will stage the footage and edit proposals. Every result lands on your timeline to edit or undo."}</div><div className="rounded-lg border border-primary/25 bg-primary/[0.07] p-3"><div className="flex items-center gap-1.5 font-medium text-primary"><Sparkles size={12} /> {zh ? "可审阅的操作" : "Reviewable operations"}</div><div className="mt-2 space-y-1.5 text-[10px] text-white/50"><span className="flex items-center gap-1.5"><Check size={11} className="text-primary" />{zh ? "素材与项目留在本机" : "Media and projects stay local"}</span><span className="flex items-center gap-1.5"><Check size={11} className="text-primary" />{zh ? "每一步可回退" : "Every step is reversible"}</span></div></div></div>
        ...
        <section className="min-w-0 bg-[oklch(0.175_0.012_150)] p-2.5 sm:p-3" aria-label={zh ? "Recut Editor 演示" : "Recut Editor demo"}>
          ...
          <div className="grid min-h-[21rem] grid-cols-[7.6rem_minmax(0,1fr)_7.8rem] border-x border-white/10">
