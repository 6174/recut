...
export function AppUpdateAllControl({ apps, onUpdated }: { apps: ManagedApp[]; onUpdated: () => void | Promise<void> }) {
  ...
  async function updateAll() {
    ...
    try {
      ...
      await onUpdated();
    } catch (cause) { setMessage(`${messageOf(cause)}。${t("workspace", useLocaleStore.getState().locale, "service.diag.suffix")}`); } finally { setWorking(false); }
  }
