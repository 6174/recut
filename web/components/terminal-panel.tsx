 ...
 * [OUTPUT]: 对外提供 TerminalPanel，用于引导启动 Daemon、检测 CLI、启动和恢复本机终端会话
 * [POS]: components 的终端适配器；只传递 PTY 字节流，不理解 Codex 或 Claude 的消息格式
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */
"use client";

import { Check, Clock3, Copy, Play, Search, SquareTerminal, X } from "lucide-react";
import { useEffect, useRef, useState } from "react";

import { Button } from "@/components/ui/button";

type Agent = { id: string; name: string; command: string; available: boolean };
type Session = { id: string; projectId: string; command: string; running: boolean; startedAt: string };
...
export function TerminalPanel({ apiBase, online, projectID }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const terminal = new Terminal({ cursorBlink: true, fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: 11, theme: { background: "#0a0a0a" } });
      ...
      stream.addEventListener("output", (event) => terminal.write(JSON.parse((event as MessageEvent<string>).data)));
      const resize = new ResizeObserver(() => { fit.fit(); void send("resize", { cols: terminal.cols, rows: terminal.rows }); });
      resize.observe(host.current); terminal.focus();
  ...
  async function start(command: string) {
    ...
    try {
      const response = await fetch(`${apiBase}/v1/terminals`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId: projectID, command }) });
