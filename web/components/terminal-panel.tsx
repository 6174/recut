...
export function TerminalPanel({ apiBase, online, projectID }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const terminal = new Terminal({ cursorBlink: sessionIsLive, disableStdin: !sessionIsLive, fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: 11, theme: { background: getComputedStyle(document.documentElement).getPropertyValue("--terminal").trim() } });
