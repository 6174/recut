...
// Generation Studio 的 local-gen 等），不再硬编码单个 provider id。
...
function ProviderSettings() {
  ...
  const localProviderIDs = useMemo(() => new Set(providers.filter((item) => item.protocol === "local").map((item) => item.id)), [providers]);
...
function ProviderPicker({ onChoose, providers, selectedID }: { onChoose: (id: string) => void; providers: Provider[]; selectedID: string }) {
  const { t } = useI18n();
  const selectableProviders = providers.filter((item) => item.protocol !== "local");
...
function ModelRouteCard({ capability, connectedProviderIDs, localProviderIDs, models, onChoose, onConnect, providerName, selectedID }: { capability: (typeof capabilities)[number]; connectedProviderIDs: Set<string>; localProviderIDs: Set<string>; models: Model[]; onChoose: (capability: string, modelID: string) => void; onConnect: (provider?: string) => void; providerName: (id: string) => string; selectedID?: string }) {
  const { t } = useI18n();
  const Icon = capability.icon;
  const label = t(capability.labelKey);
  // 候选只保留已连接凭据的 provider + 本机免 key 的本地 provider（local-audio / local-gen 等）；目录里未连接的
  // provider（历史/未购买）不出现，避免「分组一大堆但配不了」的噪音。
  const connected = models.filter((model) => connectedProviderIDs.has(model.provider) || localProviderIDs.has(model.provider));
