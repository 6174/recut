 ...
 * [OUTPUT]: 对外提供全局设置面板，以及本机/LAN service 连接、全局对话引导、Recut Skill 软链接、Recut MCP 工具清单、Provider 连接和用途模型配置体验；加载完成前保留明确等待态，图片用途可选择无需密钥的 Codex 原生生图，表单字段均有可见标签
 * [POS]: web/components 的工作台级设置入口；service 地址、全局 Agent 引导、Recut Skill、Provider、Codex 原生图片与用途模型的唯一用户配置界面，API Key 草稿不外泄到全局缓存
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */
"use client";

import { AppWindow, Bot, Check, ChevronDown, Copy, Image, Link2, Map, Mic2, Plug, Plus, Server, Settings, Sparkles, TerminalSquare, Video, X } from "lucide-react";
...
type SettingSection = "service" | "apps" | "cli" | "agents" | "onboarding" | "multimodal" | "skill" | "mcp";
        ...
        <div aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-foreground/20 p-6 backdrop-blur-[1px]" onMouseDown={() => setOpen(false)} role="dialog">
          <section className="grid h-[min(760px,calc(100vh-3rem))] w-full max-w-6xl overflow-hidden rounded-sm border bg-card shadow-2xl [grid-template-columns:224px_minmax(0,1fr)]" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="min-w-0 overflow-y-auto p-8">
              <div className="flex items-start justify-between border-b pb-6"><div><h2 className="text-lg font-semibold">{activeSection === "multimodal" ? "AI 服务商" : sections.find((item) => item.id === activeSection)?.label}</h2><p className="mt-1.5 max-w-2xl text-sm text-muted-foreground">{activeSection === "service" ? "选择此工作台要连接的 Recut service。默认仍是本机地址。" : activeSection === "multimodal" ? "先连接 Provider，再为每种创作用途选择合适的模型。密钥只在本机加密保存。" : activeSection === "onboarding" ? "为没有专属 App 引导的对话补充常用起点。点击卡片后只会写入你定义的提示词。" : activeSection === "skill" ? "把由 Recut service 持续维护的 Skill 链接到你日常使用的 Agent。" : activeSection === "mcp" ? "本机 service 通过 MCP Host 提供的全部工具：平台能力始终可见，App 操作按其归属分组。" : "此设置将在后续版本开放。"}</p></div><button aria-label="关闭设置" className="grid size-8 place-items-center rounded-xs text-muted-foreground hover:bg-muted" onClick={() => setOpen(false)} type="button"><X className="size-4" /></button></div>
              {activeSection === "service" ? <ServiceEndpointSettings /> : activeSection === "multimodal" ? <ProviderSettings /> : activeSection === "onboarding" ? <AgentOnboardingSettings apiBase={apiBase} /> : activeSection === "skill" ? <RecutSkillSettings apiBase={apiBase} /> : activeSection === "mcp" ? <RecutMCPSettings apiBase={apiBase} /> : <div className="grid min-h-80 place-items-center text-xs text-muted-foreground">即将推出</div>}
            </div>
          </section>
        </div>
      )}
