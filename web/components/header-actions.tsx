/*
 * [INPUT]: 依赖 lucide-react 的 Github 图标、ServiceControl 的 service 状态与操作、含 Recut Skill Tab 的 SettingsPanel 全局设置交互、工作台 i18n 字典
...
export function HeaderActions({ children, onSettingsOpenChange, settingsOpen, settingsSection }: HeaderActionsProps) {
  ...
  return <div className="ml-4 flex shrink-0 items-center gap-2 font-mono text-[10px] text-muted-foreground">
    ...
    <a aria-label={t("service.github.aria")} className="grid size-8 place-items-center rounded-sm transition-colors hover:bg-muted hover:text-foreground" href="https://github.com/6174/recut" rel="noreferrer" target="_blank" title={t("service.github.aria")}>
