 ...
 * [OUTPUT]: 对外提供按 App ID 解析的 appIcon 与统一暗色应用图标徽标 AppIdentityIcon
...
export function AppIdentityIcon({ appID, className = "", iconClassName = "" }: { appID: string; className?: string; iconClassName?: string }) {
  ...
  return <span className={`grid size-11 shrink-0 place-items-center rounded-xl border bg-muted text-muted-foreground ${className}`}><Icon aria-hidden="true" className={`size-5 ${iconClassName}`} strokeWidth={1.8} /></span>;
}
