...
next.config.ts: 前端构建配置；非静态导出时按 Host 将 `localhost` 根路径服务端重写到 `/marketing`，保持本地官网与 App 域名语义且不产生水合闪烁。
wrangler.toml: `recut-web` Cloudflare Worker 与静态 Assets 发布配置；以可审阅的 `custom_domain` 路由声明 `recut.video`、`www.recut.video` 与 `app.recut.video`，部署时由 Cloudflare 管理 DNS 和 Universal SSL 绑定。
worker.ts: Cloudflare 静态站边缘入口；按 Host 将 `recut.video` / `www.recut.video` 根路径映射为公开官网 Landing，将 `app.recut.video` 映射为创作工作台；同时在边缘内部将 World、项目和 App 语义深链分别映射为唯一静态壳，浏览器仍保留真实 URL，直接交给静态 Assets binding，绝不代理 localhost service。
