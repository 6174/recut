...
marketing-apps.ts: 官网应用市场的独立营销数据源；提供 `MarketingApp` 类型、`marketingApps` 目录与 `getMarketingApp`，每个 App 携带关键词、SEO tagline/description、板块正文（sections）、卖点、FAQ、相关应用内链与可选设备要求；App 的 `id` 需与工作台 Catalog 的 app id 一致以打通深链，但内容与安装目录完全解耦，官网 SEO 页面只消费它。
...
marketing-posts.ts: 官网 Blog 的无 UI 静态文章目录；导出 `MarketingPost` 类型并携带每篇唯一 markdown 正文，供 `/blog/[slug]` 的构建期参数生成、逐篇元数据、sitemap、JSON-LD 和客户端 Marketing 展示共同消费，避免客户端组件成为静态路由的数据源。
