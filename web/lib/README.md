...
recut-worlds-client.ts: Creation Worlds 的浏览器传输适配器；请求/响应与全局 SDK 及 MCP 同构，World/Entity 分页读取、创建/修改/Reference/Resolve 与项目 World Context 读写；错误统一解包为结构化 `RecutWorldsError`；只被原生 Recut 页面使用，App iframe 永不经它。
