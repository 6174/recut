import { CORNER_RADIUS_MIN } from "@timeline/text/background";
import { DEFAULTS } from "@timeline/timeline/defaults";
import type { TextElement } from "@timeline/timeline";
import type { TextBackground } from "@timeline/text/background";
import { resolveNumberAtTime } from "@timeline/animation/values";
import {
	getTextVisualRect,
} from "./layout";
import {
...
export function buildTextLayoutParamsFromElement({
	...
	return {
		...
		strokeWidth: element.params["stroke.enabled"] === true
			? readNumberParam({
					params: element.params,
					key: "stroke.width",
