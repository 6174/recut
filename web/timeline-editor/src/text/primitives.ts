import type { TextCanvasContext, TextBlockMeasurement } from "@timeline/text/layout";
import { DEFAULTS } from "@timeline/timeline/defaults";
import { clamp } from "@timeline/utils/math";
...
export interface TextLayoutParams {
	content: string;
	fontSize: number;
	/** 文本盒宽度（canvas 逻辑像素，0/缺省=自动宽度不换行）。 */
	boxWidth?: number;
	fontFamily: string;
	...
	return {
		...
		scaledStrokeWidth:
			text.strokeWidth && text.strokeWidth > 0
				? text.strokeWidth * (canvasHeight / FONT_SIZE_SCALE_REFERENCE)
				: 0,
		strokeColor: text.strokeColor ?? "#000000",
	};
}

function boxWidthToPx({
	boxWidth,
	canvasHeight,
}: {
	boxWidth?: number;
	...
	if (!boxWidth || boxWidth <= 0) return null;
...
function wrapTextLines({
	...
	if (boxWidthPx == null) return paragraphs;
	const out: string[] = [];
	for (const para of paragraphs) {
		...
		for (const ch of para) {
			const trial = line + ch;
			const w = trial === "" ? 0 : ctx.measureText(trial).width;
			if (line !== "" && w > boxWidthPx) {
				out.push(line);
				line = ch === " " ? "" : ch;
			} else {
				line = trial;
...
export function measureTextLayout({
	...
	const lineMetrics = lines.map((line) =>
		line === "" ? ctx.measureText(" ") : ctx.measureText(line),
	);
	ctx.restore();
	...
	// 盒模式：布局宽度锁定为盒宽（而非最长行宽），背景框/选中框/对齐都以盒宽
	// 为准 —— 几何只由 boxWidth 决定，拖拽手柄严格跟手、换行不引起横向跳动。
	if (boxWidthPx != null) {
		block.maxWidth = boxWidthPx;
	}
...
export function drawMeasuredTextLayout({
	...
	const shouldStroke =
		layout.scaledStrokeWidth > 0 &&
		layout.strokeColor &&
		layout.strokeColor !== "transparent";
	if (shouldStroke) {
		ctx.strokeStyle = layout.strokeColor;
		ctx.lineWidth = layout.scaledStrokeWidth;
		ctx.lineJoin = "round";
		ctx.lineCap = "round";
	}
