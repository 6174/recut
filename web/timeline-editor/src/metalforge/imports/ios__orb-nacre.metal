...
static float nc_sstep(float e0, float e1, float x) {
    float t = clamp((x - e0) / (e1 - e0), 0.0, 1.0);
    return t * t * (3.0 - 2.0 * t);
}

static float nc_hash(float3 p) {
    return fract(sin(dot(p, float3(127.1, 311.7, 74.7))) * 43758.5453);
}

static float nc_noise(float3 p) {
    float3 i = floor(p);
    float3 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = nc_hash(i);
...
[[ stitchable ]] half4 orbNacre(float2 position,
                                half4  color,
                                float4 boundingRect,
                                float  time,
                                float  speed,
    ...
    if (r < rad + 0.01 + mf_edge_d(edgeSoftness)) {
        ...
        float3 fr = select(0.5 - 0.5 * cos(6.2831 * th * freq),
