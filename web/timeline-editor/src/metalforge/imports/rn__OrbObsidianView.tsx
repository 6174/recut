...
const float stone = 0.42;
const float relief = 0.14;
const float grain = 0.045;
const float sheen = 7.0;
const float glow = 1.0;
const float exposure = 1.0;
const float edgeSoftness = 0.005;
const float edgeGlow = 0.0;
const float paletteCount = 0.0;
const float2 center = float2(0.0, 0.06);
const half4 tint = half4(0.54902, 0.439216, 1.0, 1.0);
const half4 rimColor = half4(0.6, 0.521569, 1.0, 1.0);
const half4 glassColor = half4(0.6, 0.760784, 1.0, 1.0);
...
const half4 lampAColor = half4(0.776471, 0.815686, 1.0, 1.0);
const half4 lampBColor = half4(0.592157, 0.682353, 1.0, 1.0);
const half4 specColor = half4(1.0, 0.996078, 0.988235, 1.0);
const half4 softboxColor = half4(0.94902, 0.968627, 1.0, 1.0);
const half4 lobeColor = half4(0.819608, 0.901961, 1.0, 1.0);
const half4 limbColor = half4(0.85098, 0.921569, 1.0, 1.0);
const half4 keyColor = half4(1.0, 0.980392, 0.941176, 1.0);
const half4 keyMidColor = half4(0.901961, 0.94902, 1.0, 1.0);
const half4 keyWideColor = half4(0.721569, 0.819608, 1.0, 1.0);
const half4 envColor = half4(0.101961, 0.258824, 0.701961, 1.0);
const half4 envTint = half4(0.619608, 0.2, 0.8, 1.0);
const half4 ambientColor = half4(0.701961, 0.65098, 1.0, 1.0);
const half4 stoneSpecColor = half4(1.0, 1.0, 1.0, 1.0);
const half4 glowColor = half4(0.54902, 0.439216, 1.0, 1.0);
const half4 paletteStop0 = half4(0.713725, 0.768627, 1.0, 1.0);
const half4 paletteStop1 = half4(0.709804, 0.760784, 1.0, 1.0);
const half4 paletteStop2 = half4(0.701961, 0.752941, 1.0, 1.0);
const half4 paletteStop3 = half4(0.694118, 0.741176, 1.0, 1.0);
const half4 paletteStop4 = half4(0.682353, 0.72549, 1.0, 1.0);
const half4 paletteStop5 = half4(0.666667, 0.701961, 1.0, 1.0);
const half4 paletteStop6 = half4(0.643137, 0.662745, 1.0, 1.0);
const half4 paletteStop7 = half4(0.592157, 0.592157, 1.0, 1.0);
const half4 paletteStop8 = half4(0.592157, 0.592157, 1.0, 1.0);
const half4 paletteStop9 = half4(0.592157, 0.592157, 1.0, 1.0);
const half4 paletteStop10 = half4(0.592157, 0.592157, 1.0, 1.0);
const half4 paletteStop11 = half4(0.592157, 0.592157, 1.0, 1.0);
...
float ob_sstep(float e0, float e1, float x) {
    float t = clamp((x - e0) / (e1 - e0), 0.0, 1.0);
    return t * t * (3.0 - 2.0 * t);
}

float3 ob_refract(float3 i, float3 n, float eta) {
    float d = dot(n, i);
...
float3 ob_rotY(float3 p, float a) {
    float c = cos(a), s = sin(a);
    return float3(c * p.x + s * p.z, p.y, -s * p.x + c * p.z);
}

float3 ob_rotX(float3 p, float a) {
    float c = cos(a), s = sin(a);
    return float3(p.x, c * p.y - s * p.z, s * p.y + c * p.z);
}

float3 ob_aces(float3 x) {
    return clamp((x * (2.51 * x + 0.03)) / (x * (2.43 * x + 0.59) + 0.14), 0.0, 1.0);
}

float ob_sphExit(float3 p, float3 d) {
    float b = dot(p, d);
    return -b + sqrt(max(1.0 - dot(p, p) + b * b, 0.0));
}

float3 ob_knead(float t, float a, float b, float c, float ph) {
    return float3(sin(t * a) + 0.55 * sin(t * a * 0.41 + ph),
                  cos(t * b) + 0.55 * cos(t * b * 0.37 + ph * 1.7),
                  sin(t * c + ph * 2.3));
}

float ob_schlick(float ct, float f0) {
    return f0 + (1.0 - f0) * pow(max(1.0 - ct, 0.0), 5.0);
}

float3 ob_studioBG(float2 p, float3 wallA, float3 wallB,
                   float3 lampA, float3 lampB) {
    float3 wall = mix(wallA * 0.0112, wallB * 0.0027, smoothstep(-0.55, 1.25, p.y));
...
float3 ob_bgThrough(float2 uv, float3 N, float3 wallA, float3 wallB,
                    float3 lampA, float3 lampB) {
    ...
    return float3(ob_studioBG(uv + d1.xy * 1.15, wallA, wallB, lampA, lampB).r,
                  ob_studioBG(uv + d2.xy * 1.15, wallA, wallB, lampA, lampB).g,
                  ob_studioBG(uv + d3.xy * 1.15, wallA, wallB, lampA, lampB).b) * 2.6;
}

float3 ob_glassHi(float3 N, float2 su, float z, float3 glassCol,
...
float3 ob_envMirror(float2 uv, float3 R, float keyP, float keyI,
    ...
    float3 e = ob_studioBG(uv * 0.55 + R.xy * 0.72, wallA, wallB, lampA, lampB) * 7.5;
...
float ob_sdf(float3 p, float t, float sr, float rl, float gr) {
...
float3 ob_nrm(float3 p, float t, float sr, float rl, float gr) {
    float2 e = float2(0.004, 0.0);
    return normalize(float3(
        ob_sdf(p + e.xyy, t, sr, rl, gr) - ob_sdf(p - e.xyy, t, sr, rl, gr),
        ob_sdf(p + e.yxy, t, sr, rl, gr) - ob_sdf(p - e.yxy, t, sr, rl, gr),
        ob_sdf(p + e.yyx, t, sr, rl, gr) - ob_sdf(p - e.yyx, t, sr, rl, gr)));
}
...
half4 main(float2 position) {
    ...
    float3 col = ob_studioBG(uv, wallA, wallB, lampA, lampB);
    ...
    if (r < 1.004 + mf_edge_d(edgeSoftness)) {
        ...
        float3 bg = ob_bgThrough(uv, N, wallA, wallB, lampA, lampB);
        ...
        if (hit > 0.5) {
            ...
            float3 e = ob_envMirror(uv, R, 1400.0, sheen, float3(bounceColor.rgb),
                                    ...
                                    float3(envColor.rgb), float3(envTint.rgb));
    ...
    float3 edged = mf_edge_glow(clamp(col, 0.0, 1.0), uv, center, max(radius, 0.05),
