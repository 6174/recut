...
const float2 core = float2(0.2, 0.14);
const half4 deepColor = half4(0.160784, 0.019608, 0.078431, 1.0);
const half4 midColor = half4(0.721569, 0.121569, 0.219608, 1.0);
const half4 flameColor = half4(1.0, 0.45098, 0.121569, 1.0);
const half4 hotColor = half4(1.0, 0.839216, 0.521569, 1.0);
const half4 coreColor = half4(1.0, 0.721569, 0.4, 1.0);
const half4 haloColor = half4(0.74902, 0.25098, 0.14902, 1.0);
const half4 haloTintColor = half4(0.85098, 0.45098, 0.2, 1.0);
const half4 emberGlowColor = half4(1.0, 0.4, 0.180392, 1.0);
const half4 ambientColor = half4(0.05098, 0.011765, 0.031373, 1.0);
const half4 rimColor = half4(1.0, 0.4, 0.219608, 1.0);
const half4 rimTintColor = half4(0.85098, 0.2, 0.45098, 1.0);
const half4 glowColor = half4(1.0, 0.45098, 0.121569, 1.0);
const half4 paletteStop0 = half4(0.160784, 0.019608, 0.078431, 1.0);
const half4 paletteStop1 = half4(0.270588, 0.039216, 0.105882, 1.0);
const half4 paletteStop2 = half4(0.611765, 0.101961, 0.192157, 1.0);
const half4 paletteStop3 = half4(0.729412, 0.129412, 0.215686, 1.0);
const half4 paletteStop4 = half4(0.905882, 0.341176, 0.152941, 1.0);
const half4 paletteStop5 = half4(1.0, 0.454902, 0.12549, 1.0);
const half4 paletteStop6 = half4(1.0, 0.733333, 0.411765, 1.0);
const half4 paletteStop7 = half4(1.0, 0.839216, 0.521569, 1.0);
const half4 paletteStop8 = half4(1.0, 0.839216, 0.521569, 1.0);
const half4 paletteStop9 = half4(1.0, 0.839216, 0.521569, 1.0);
const half4 paletteStop10 = half4(1.0, 0.839216, 0.521569, 1.0);
const half4 paletteStop11 = half4(1.0, 0.839216, 0.521569, 1.0);
...
float nb_sstep(float e0, float e1, float x) {
    float t = clamp((x - e0) / (e1 - e0), 0.0, 1.0);
    return t * t * (3.0 - 2.0 * t);
}

float nb_hash(float3 p) {
    return fract(sin(dot(p, float3(127.1, 311.7, 74.7))) * 43758.5453);
}

float nb_noise(float3 p) {
    float3 i = floor(p);
    float3 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = nb_hash(i);
...
float3 nb_pal(float x, float3 c0, float3 c1, float3 c2, float3 c3) {
    ...
    float3 acc = float3(0.0);
        ...
        for (int i = 0; i < N; i++) {
            ...
            float3 c  = (paletteCount > 0.5
                           ? mf_ramp_linR(px, pal)
                           : nb_pal(px, float3(deepColor.rgb), float3(midColor.rgb),
                                    float3(flameColor.rgb), float3(hotColor.rgb))) * lit;
            acc += T * c * aa * 2.0;
            T   *= 1.0 - aa * 0.86;
        }
