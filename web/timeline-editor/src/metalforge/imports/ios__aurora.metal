...
static inline float fieldAt(texture2d<float> tex, float2 uv) {
    float3 c = tex.sample(fluidSampler, uv).rgb;
...
fragment float4 wind_fragment(VSOut in [[stage_in]],
                              texture2d<float> uVelocity [[texture(0)]],
                              texture2d<float> uDye      [[texture(1)]],
                              constant float &strength [[buffer(0)]],
                              constant float &scale    [[buffer(1)]],
                              constant float &lift     [[buffer(2)]],
                              constant float &time     [[buffer(3)]],
                              constant float &dt       [[buffer(4)]]) {
    float2 uv = in.uv;
    float2 velocity = uVelocity.sample(fluidSampler, uv).xy;
    float mass = fieldAt(uDye, uv);

    float shear = sin(uv.y * scale + time * 0.55)
                + 0.55 * sin(uv.y * scale * 2.31 - time * 0.83);
    velocity.x += shear * strength * dt;
    velocity.y -= mass * lift * dt;
...
fragment float4 display_aurora_fragment(VSOut in [[stage_in]],
                                        texture2d<float> uTexture [[texture(0)]],
                                        texture2d<float> uBloom   [[texture(1)]],
                                        constant DisplayUniforms &u [[buffer(0)]]) {
    float2 uv = in.uv;

    float3 c = mix(float3(u.ramp0r, u.ramp0g, u.ramp0b),
                   float3(u.ramp1r, u.ramp1g, u.ramp1b),
                   smoothstep(0.0, 0.85, uv.y));

    for (int i = 0; i < 10; ++i) {
        float f = float(i) / 9.0;
        float2 slice = uv + float2(sin(u.time * 0.27 + f * 4.1 + uv.y * 7.0) * 0.012 * f,
                                   -f * u.p1);
        float d = fieldAt(uTexture, slice);

        float3 tint = mix(float3(u.ramp2r, u.ramp2g, u.ramp2b),
                          float3(u.ramp3r, u.ramp3g, u.ramp3b), f);
        c += tint * d * pow(1.0 - f, 1.5) * u.p0;
    }
