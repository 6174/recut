...
const float haloRim = 26.0;
const float haloMid = 34.0;
const float haloWash = 44.0;
const float haloOuter = 62.0;
const float haloCore = 1.0;
const float grainAmt = 0.16;
const float intensity = 1.0;
const float shadowAmt = 0.1;
const float2 card = float2(0.86, 0.52);
const float2 light = float2(0.5, 1.0);
const half4 bgTop = half4(0.098039, 0.094118, 0.12549, 1.0);
const half4 bgBottom = half4(0.098039, 0.094118, 0.12549, 1.0);
const half4 rimColor = half4(0.87451, 0.811765, 0.972549, 1.0);
const half4 midColor = half4(0.439216, 0.321569, 1.0, 1.0);
const half4 deepColor = half4(0.082353, 0.419608, 0.988235, 1.0);
const half4 topColor = half4(0.023529, 0.133333, 0.466667, 1.0);
const half4 accentColor = half4(0.078431, 0.07451, 0.12549, 1.0);
const half4 shadowColor = half4(0.439216, 0.321569, 1.0, 1.0);
...
const float G_H1_OFF = 0.03846154;
const float G_H1_SPREAD = 0.11538462;
const float G_H2_OFF = 0.17647059;
const float G_H2_SPREAD = 0.23529412;
const float G_H3_OFF = 0.0;
const float G_H3_SPREAD = 0.27272727;
const float G_H4_OFF = 0.01612903;
const float G_H4_SPREAD = 0.32258065;

const float G_H_BLUR = 1.0;

const float G_HA1 = 0.8;
const float G_HA2 = 1.0;
const float G_HA3 = 1.0;
const float G_HA4 = 1.0;

const float G_CORE_RX = 185.6;
const float G_CORE_RY = 218.4;
const float G_CORE_A0 = 1.00;
const float G_CORE_A1 = 0.92;
const float G_CORE_S1 = 0.46;
const float G_CORE_S2 = 0.82;
...
float3 mfSrc(float2 cuv) {
    ...
    float a4 = g_inset_band(q, halfExt, r, -ldir * G_H4_OFF, haloOuter, G_H_BLUR, G_H4_SPREAD, gs) * G_HA4 * inten;
    cardCol = g_over(cardCol, float3(topColor.rgb), a4);
    float a3 = g_inset_band(q, halfExt, r, -ldir * G_H3_OFF, haloWash, G_H_BLUR, G_H3_SPREAD, gs) * G_HA3 * inten;
    cardCol = g_over(cardCol, float3(deepColor.rgb), a3);
    float a2 = g_inset_band(q, halfExt, r, -ldir * G_H2_OFF, haloMid, G_H_BLUR, G_H2_SPREAD, gs) * G_HA2 * inten;
    cardCol = g_over(cardCol, float3(midColor.rgb), a2);
    float a1 = g_inset_band(q, halfExt, r, -ldir * G_H1_OFF, haloRim, G_H_BLUR, G_H1_SPREAD, gs) * G_HA1 * inten;
    cardCol = g_over(cardCol, float3(rimColor.rgb), a1);

    float2 coreRad = float2(G_CORE_RX, G_CORE_RY) * gs * max(haloCore, 0.0001);
    float e = length(q / coreRad);
    cardCol = g_over(cardCol, float3(accentColor.rgb), g_halo_core(e));
