// Ink Bloom Orb — near-black billows silhouetted on a glowing interior.
//
...
// WGSL then aligns `haloColor` to a 16-byte boundary — the packer mirrors that.
// New floats are appended after `exposure` and new colours after
// `rimTintColor`, never inserted.
struct Uniforms {
  size:          vec2<f32>,
  time:          f32,
  speed:         f32,
...
// WGSL leaves that indeterminate, so all three places spell the formula out.
...
fn orbInkBloomAnim(uv01: vec2<f32>) -> vec4<f32> {
  ...
  if (r < rad + 0.01 + mfEdgeD(u.edgeSoftness)) {
    ...
    // Two incommensurate drifts: k1 kneads the base field, k2 the warp that
    // rides on it. Neither period divides the other, so the ink never loops.
    ...
    // The fringe's three cosines run a fixed ratio apart — the interference
    // triplet, and where nearly all of this orb's colour is born. `spectrum`
    // interpolates those ratios toward one shared frequency: at 1 the vector is
    // exactly the literal triplet below, at 0 all three channels band together
    // and the fringe turns greyscale with its pattern intact, above 1 they
    // separate further. Loop-invariant, so it is built once.
    ...
    for (var i: i32 = 0; i < N; i = i + 1) {
      ...
      // The slice tapers as it recedes, so the billows read as a solid.
      let p  = vec3<f32>(su * (1.0 - 0.24 * (z - fz)), fz);
      let rr = length(p);
      let g  = ibFbm(p * 2.0 + k1);
      let w  = ibFbm(p * 1.35 + vec3<f32>(g * 1.9) + k2);
      let msk  = ibSstep(1.0, 0.6, rr);
      // Body absorbs; edge — a thin band astride w = 0.5 — emits.
      let body = smoothstep(0.50, 0.68, w) * msk;
      let edge = pow(ibSstep(0.10, 0.0, abs(w - 0.50)), 2.0) * msk;
      // Three cosines a fixed ratio apart: the interference triplet, tinted by
      // `filmColor` (white by default, so a no-op until the user says otherwise).
    ...
    // Whatever the ink did not absorb sees the interior, a diagonal ramp
    // brightened towards the middle of the disc.
    ...
    // The limb's fresnel ramps laterally between two tints — both editable,
    // defaulting to the literals this shader shipped with.
