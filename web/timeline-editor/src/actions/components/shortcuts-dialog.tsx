...
} from "@timeline/actions/use-keyboard-shortcuts-help";
import { useKeybindingsStore } from "@timeline/actions/keybindings-store";
import { getActionDefinition } from "@timeline/actions";
import { t, useRecutLocale } from "@timeline/i18n";
import { localizeActionCategory, localizeActionDescription } from "@timeline/actions/i18n";
	...
	useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (keyString) {
				...
				if (conflict) {
					toast.error(
						t(locale, "shortcut.bound", {
							key: keyString,
							action: localizeActionDescription({
								description:
									getActionDefinition({ action: conflict.existingAction })
										.description ?? conflict.existingAction,
								locale,
							}),
						}),
					);
	...
	return (
		<Dialog open={isOpen} onOpenChange={onOpenChange}>
			<DialogContent className="flex max-h-[80vh] max-w-2xl flex-col p-0">
				<DialogHeader>
					<DialogTitle>{t(locale, "shortcut.title")}</DialogTitle>
				</DialogHeader>
						...
						{categories.map((category) => (
							<div key={category} className="flex flex-col gap-1">
								<h3 className="text-muted-foreground text-xs font-medium tracking-wide uppercase">
									{localizeActionCategory({ category, locale })}
								</h3>
	...
	return (
		<div className="flex items-center justify-between">
			<div className="flex items-center gap-3">
				...
				<span className="text-sm">{localizeActionDescription({ description: shortcut.description, locale })}</span>
			</div>
				...
				{displayKeys.map((key: string, index: number) => (
					<div key={key} className="flex items-center gap-2">
						...
						{index < displayKeys.length - 1 && (
							<span className="text-muted-foreground text-xs">{t(locale, "shortcut.or")}</span>
						)}
					</div>
				))}
			</div>
