/**
 * [INPUT]: 依赖时间线书签状态、Popover 与全局颜色和数值表单控件。
 * [OUTPUT]: 对外提供书签时间、注释、颜色和持续时间的交互组件。
 * [POS]: timeline/bookmarks/components 的书签编辑与渲染入口，属性输入复用编辑器统一标准。
...
import { DEFAULT_TIMELINE_BOOKMARK_COLOR } from "@timeline/timeline/components/theme";
import { TIMELINE_BOOKMARK_ROW_HEIGHT_PX } from "@timeline/timeline/components/layout";
...
import {
	...
	PopoverAnchor,
	PopoverContent,
} from "@timeline/components/ui/popover";
...
import { clamp, formatNumberForDisplay } from "@timeline/utils/math";
import { timelineTimeToPixels, timelineTimeToSnappedPixels } from "@timeline/timeline";
import {
	type MediaTime,
		...
		<Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
			<PopoverAnchor asChild>
				<button
					...
					aria-label={t(locale, "ui.selectBookmark", { time: formatNumberForDisplay({ value: mediaTimeToSeconds({ time }), fractionDigits: 1 }) })}
					type="button"
	...
	return (
		<>
			<div className="flex flex-col gap-2">
				<Label className="text-xs">{t(locale, "ui.note")}</Label>
				<Input
					placeholder={t(locale, "ui.addNote")}
					value={bookmark.note ?? ""}
			...
			<div className="flex flex-col gap-2">
				<Label className="text-xs">{t(locale, "ui.color")}</Label>
					...
					{bookmark.color &&
						bookmark.color.replace(/^#/, "").toUpperCase() !==
							...
							<Button
								...
								variant="text"
								size="text"
								aria-label={t(locale, "ui.resetColor")}
			...
			<div className="flex flex-col gap-2">
				<Label className="text-xs">{t(locale, "common.duration")}</Label>
						...
						onScrub={(value) =>
							handleUpdate({
								duration: mediaTimeFromSeconds({
									seconds: clamp({ value, min: 0, max: maxDuration }),
								}),
							})
						}
					/>
			...
			<Button
				...
				aria-label={t(locale, "ui.deleteBookmark")}
			>
