...
import { processMediaAssets } from "@timeline/media/processing";
import { showMediaUploadToast } from "@timeline/media/upload-toast";
import {
	DEFAULT_NEW_ELEMENT_DURATION,
	toElementDurationTicks,
} from "@timeline/timeline/creation";
import { BASE_TIMELINE_PIXELS_PER_SECOND } from "@timeline/timeline/scale";
...
import { AddTrackCommand, InsertElementCommand } from "@timeline/commands/timeline";
import { BatchCommand } from "@timeline/commands";
import type { Command } from "@timeline/commands/base-command";
import { computeDropTarget } from "@timeline/timeline/components/drop-target";
import { getTimelineDisplayTracks } from "@timeline/timeline/components/track-layout";
import type { TimelineDragSource } from "@timeline/timeline/drag-source";
import type {
	TrackType,
	DropTarget,
	ElementType,
...
import type { TimelineDragData } from "@timeline/timeline/drag";
import type { MediaAsset } from "@timeline/media/types";
import type { ProcessedMediaAsset } from "@timeline/media/processing";
import { roundFrameTime, type MediaTime } from "@timeline/wasm";
import { parseSubtitleFile } from "@timeline/subtitles/parse";
import { insertCaptionChunksAsTextTrack } from "@timeline/subtitles/insert";
import { EditorCore } from "@timeline/core";

const SUBTITLE_EXTENSIONS = new Set(["srt", "ass"]);

function isSubtitleFileName(file: File): boolean {
...
function orderedTracks({
	...
	return getTimelineDisplayTracks({ tracks: sceneTracks });
}
...
export class DragDropController {
	...
	onDragOver(event: DragEvent): void {
		...
		if (!dragData) {
			if (hasFiles && isExternal) {
				const subtitleFiles = Array.from(
					event.dataTransfer.files ?? [],
				).filter(isSubtitleFileName);
				if (subtitleFiles.length > 0) {
					// 字幕文件拖放：显示文本落点定位线，落点为第一条 cue 起点。
					const target = computeDropTarget({
						...
						isExternalDrop: true,
						elementDuration: DEFAULT_NEW_ELEMENT_DURATION,
						pixelsPerSecond: BASE_TIMELINE_PIXELS_PER_SECOND,
						zoomLevel: this.config.zoomLevel,
					});
					this.setOver({ dropTarget: target, elementType: "text" });
	...
	private async executeFileDrop({
		...
		const subtitleFiles = files.filter(isSubtitleFileName);
		const mediaFiles = files.filter((file) => !isSubtitleFileName(file));

		if (subtitleFiles.length > 0) {
			await this.executeSubtitleFileDrop({
				files: subtitleFiles,
				mouseX,
				mouseY,
			});
		...
		await showMediaUploadToast({
			...
			promise: async () => {
				const processedAssets = await processMediaAssets({ files: mediaFiles });
	...
	/** 把拖入的 SRT/ASS 直接铺成字幕轨（每条 cue 独立 text 元素）。 */
	private async executeSubtitleFileDrop({
		files,
		mouseX,
		mouseY,
	}: {
		...
		const startOffsetTicks = dropTarget?.xPosition ?? currentTime;
		...
		for (const file of files) {
			try {
				...
				if (result.captions.length === 0) {
					toast.error(t(getRecutLocale(), "captions.noValidCuesFile", { name: file.name }));
					continue;
				}
				const extension = file.name.split(".").pop()?.toLowerCase();
				insertCaptionChunksAsTextTrack({
					...
					options: {
						startOffsetTicks,
						source: extension === "ass" ? "ass" : "srt",
					},
				});
			} catch (error) {
				console.error("Failed to import subtitle file:", file.name, error);
				toast.error(t(getRecutLocale(), "captions.importFailed", { name: file.name }));
			}
		}
	}
}
