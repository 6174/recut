...
import { canPlaceTimeSpansOnTrack } from "@timeline/timeline/placement/overlap";
import type {
	GroupMoveResult,
	MoveGroup,
	PlannedElementMove,
...
function resolveExistingTrackMove({
	...
	const targetTrackIdsByElementId = allMembersShareSourceTrack(group)
		? new Map(
				group.members.map((member) => [
					member.elementId,
					anchorTargetPlacement.trackId,
				]),
			)
		: resolveExistingTrackIdsByElementId({
				group,
				tracks,
...
function resolveNewTrackMove({
	...
	const hasVisibleMainTrack = getDisplayTracks({ tracks }).some(
		(track) => track.id === tracks.main.id,
	);
	const fullAnchorInsertIndex =
		anchorInsertIndex +
		(!hasVisibleMainTrack && anchorInsertIndex > tracks.overlay.length ? 1 : 0);
	...
	const sharesSourceTrack = allMembersShareSourceTrack(group);
	if (
		anchorMemberIndex < 0 ||
		(!sharesSourceTrack && newTrackIds.length < sortedMembers.length) ||
		(sharesSourceTrack && newTrackIds.length < 1)
	) {
		return null;
	}

	if (sharesSourceTrack) {
		const targetTrackId = newTrackIds[0];
		...
		const blockStartIndex =
			group.anchor.trackSection === "audio"
				? clampAudioInsertIndex({
						tracks,
						insertIndex: fullAnchorInsertIndex,
					})
				: Math.max(
						0,
						Math.min(fullAnchorInsertIndex, tracks.overlay.length),
					);
		const moves = group.members.map((member) => ({
			sourceTrackId: member.trackId,
			targetTrackId,
			...
			createTracks: [
				{
					...
					type: getTrackTypeForElementType({
						elementType: group.anchor.elementType,
					}),
					index: blockStartIndex,
				},
			],
			targetSelection: moves.map(({ elementId }) => ({
				trackId: targetTrackId,
				elementId,
			})),
		};
	...
	const blockStartIndex = hasAudioMember
		? clampAudioInsertIndex({
				tracks,
				insertIndex: fullAnchorInsertIndex - anchorMemberIndex,
			})
		: Math.max(
				0,
				Math.min(fullAnchorInsertIndex - anchorMemberIndex, tracks.overlay.length),
			);
...
function allMembersShareSourceTrack(group: MoveGroup): boolean {
	return group.members.every((member) => member.trackId === group.anchor.trackId);
}
...
function clampAudioInsertIndex({
	...
	const minimumAudioInsertIndex = tracks.overlay.length + (hasVisibleMainTrack ? 1 : 0);
	return Math.max(
