...
import { useAssetsPanelStore } from "@timeline/components/editor/panels/assets/assets-panel-store";
import { AudioWaveform, WAVEFORM_GAIN_SAMPLE_COUNT } from "./audio-waveform";
import { AudioVolumeLine } from "./audio-volume-line";
import { useElementPreview } from "@timeline/timeline/hooks/use-element-preview";
import {
	useKeyframeDrag,
	type KeyframeDragState,
} from "@timeline/timeline/hooks/element/use-keyframe-drag";
import { useKeyframeSelection } from "@timeline/timeline/hooks/element/use-keyframe-selection";
import { useKeyframeBoxSelect } from "@timeline/timeline/hooks/element/use-keyframe-box-select";
import { SelectionBox } from "@timeline/selection/selection-box";
import { getElementKeyframes } from "@timeline/animation";
import {
	canElementHaveAudio,
	canElementBeHidden,
	hasElementEffects,
...
import { componentsRegistry } from "@timeline/runtime";
import {
	ContextMenu,
	ContextMenuContent,
	ContextMenuItem,
	...
	ContextMenuTrigger,
} from "@timeline/components/ui/context-menu";
import type { SelectionBoxBounds } from "@timeline/selection/types";
import type {
...
import {
	...
	getSourceAudioActionText,
	isSourceAudioEnabled,
	isSourceAudioSeparated,
} from "@timeline/timeline/audio-separation";
import { buildWaveformGainSamples, isElementMuted } from "@timeline/timeline/audio-state";
import { getTimelinePixelsPerSecond } from "@timeline/timeline";
import { buildWaveformSourceKey } from "@timeline/media/waveform-summary";
import { addMediaTime, type MediaTime, TICKS_PER_SECOND } from "@timeline/wasm";
import {
	getActionDefinition,
	type TAction,
...
import { useElementSelection } from "@timeline/timeline/hooks/element/use-element-selection";
import { buildGraphicPreviewUrl } from "@timeline/graphics";
...
import { OcShapesIcon } from "@timeline/components/icons";
import { uppercase } from "@timeline/utils/string";
import { useMemo, type ComponentProps, type ReactNode } from "react";
import type { SelectedKeyframeRef, ElementKeyframe } from "@timeline/animation/types";
...
import { getTrackTypeForElementType } from "@timeline/timeline/placement/compatibility";
...
export function TimelineElement({
	...
	// width 用对齐后的右边缘减左边缘：相邻片段共享完全相同的像素边界，
	// 否则 left(已对齐)+width(未对齐) 与下一片段 left 之间会漏出亚像素发丝缝。
	const elementWidth = Math.max(
		timelineTimeToSnappedPixels({
			time: displayedStartTime + displayedDuration,
			zoomLevel,
		}) - elementLeft,
		// 极短片段在低缩放下对齐后可能归零，保底 1 物理像素可见。
	...
	const sourceAudioLabel =
		element.type === "video"
			? getSourceAudioActionText({ element, locale })
			: t(locale, "timeline.extractAudio");
			...
			<ContextMenu>
				...
				<ContextMenuContent className="w-64">
					...
					{hasKeyframes && (
						...
						>
							{isExpanded
								? t(locale, "timeline.collapseKeyframes")
								: t(locale, "timeline.expandKeyframes")}
						</ContextMenuItem>
					)}
					{selectedElements.length === 1 && hasMediaId(element) && (
						<>
							...
							>
								{t(locale, "timeline.revealMedia")}
							</ContextMenuItem>
							<ContextMenuItem
								icon={<HugeiconsIcon icon={Exchange01Icon} />}
								disabled
							>
								{t(locale, "timeline.replaceMedia")}
							</ContextMenuItem>
						</>
					)}
					<ContextMenuSeparator />
...
function ResizeHandle({
	...
	return (
		<button
			...
			aria-label={isLeft ? t(locale, "timeline.resizeLeft") : t(locale, "timeline.resizeRight")}
		></button>
	);
}
	...
	return indicators.map((indicator) => {
		...
		return (
			<button
				...
				aria-label={t(locale, "timeline.selectKeyframe")}
			>
				<HugeiconsIcon
					icon={KeyframeIcon}
					className={cn(
...
function TextElementContent({
	...
	if (element.subtitle) {
		return (
			<div className="flex size-full items-center gap-1.5 pl-2">
				<HugeiconsIcon
					icon={CaptionsIcon}
...
function ComponentElementContent({
	element,
}: {
	element: Extract<TimelineElementType, { type: "component" }>;
}) {
	const locale = useRecutLocale();
	...
	const name = element.name || definition?.name || t(locale, "timeline.component");
	const isEffect = definition?.category === "effect";
	return (
		<div
			className="flex size-full items-center gap-1.5 pl-2"
		...
		>
			...
			{isEffect ? (
				...
				<OcShapesIcon className="size-3.5 shrink-0 text-white/80" />
			)}
...
function TiledMediaContent({
	...
	if (mediaAsset?.status === "deleted") {
		return (
			<span className="text-destructive truncate text-xs">
				{t(locale, "timeline.resourceDeleted")}
			</span>
		);
	}
	if (mediaAsset?.status === "loading") {
		return (
			<span className="text-foreground/80 truncate text-xs">
				{t(locale, "timeline.resourceLoading")}
			</span>
		);
	}
	const imageUrl =
	...
	return (
		<div className="group/audio relative size-full">
			...
			/>
			{element.type === "video" && pixelsPerSecond !== null && (
				<div className="pointer-events-none absolute inset-x-0 top-5 bottom-0 overflow-hidden">
					<AudioWaveform
						...
						retime={element.retime}
						sourceStartSec={element.trimStart / TICKS_PER_SECOND}
						color={TIMELINE_TRACK_THEME.video.waveformColor}
					/>
				</div>
			)}
			<MediaElementHeader
			...
			{showVolumeLine && (
				<div className="absolute inset-x-0 top-5 bottom-0 overflow-hidden">
					<AudioVolumeLine
						...
						trackId={track.id}
						syncElements={
							element.type === "video" && track.type === "video"
...
function ElementContent({ element, track }: ElementContentProps) {
	switch (element.type) {
		...
		case "component":
			return <ComponentElementContent element={element} />;
		case "effect":
...
function MuteMenuItem({
	...
	return (
		<ActionMenuItem action="toggle-elements-muted-selected" icon={getIcon()}>
			{isMuted ? t(locale, "timeline.unmute") : t(locale, "timeline.mute")}
		</ActionMenuItem>
	);
}
...
function VisibilityMenuItem({
	...
	return (
		<ActionMenuItem
			...
			icon={getIcon()}
		>
			{isHidden ? t(locale, "timeline.show") : t(locale, "timeline.hide")}
		</ActionMenuItem>
	);
}
	...
	return (
		...
		>
			{isMultipleSelected && isCurrentElementSelected
				? t(locale, "timeline.deleteElements", { count: selectedCount })
				: t(
						locale,
						elementType === "text"
							? "timeline.deleteText"
