...
import { Popover, PopoverContent } from "@timeline/components/ui/popover";
...
import { getBezierPoint } from "@timeline/animation/bezier";
import type { NormalizedCubicBezier } from "@timeline/animation/types";
...
import { localizeGraphLabel, localizeGraphMessage } from "./i18n";
					...
					<div className="border-b px-3 py-2">
						<div className="bg-muted/40 inline-flex rounded-md p-0.5">
							{componentOptions.map((component) => (
								...
								>
									{localizeGraphLabel({ label: component.label, locale })}
								</button>
							))}
						</div>
					</div>
				...
				<Tabs variant="underline" defaultValue="presets" className="flex flex-col gap-2">
					<TabsList className="px-3">
						<TabsTrigger value="presets" className="text-xs">
							{t(locale, "graph.presets")}
						</TabsTrigger>
						<TabsTrigger value="saved" className="text-xs">
							{t(locale, "graph.saved")}
						</TabsTrigger>
					</TabsList>
					...
					<TabsContent value="saved" className="px-3">
						<div className="grid grid-cols-3 gap-1">
							...
							>
								...
								<span className="text-[10px] leading-tight">{t(locale, "graph.save")}</span>
							</button>
						</div>
					</TabsContent>
				</Tabs>
...
function GraphEditorEmptyState({ message }: { message: string }) {
	...
	return (
		...
		>
			{localizeGraphMessage({ message, locale })}
		</div>
	);
}
...
function PresetItem({
	...
	return (
		...
		>
			...
			>
				{preset.labelKey ? t(locale, preset.labelKey) : preset.label}
			</span>
			{onDelete && (
				<Button
					variant="destructive"
...
function CurveThumb({ value }: { value: NormalizedCubicBezier }) {
	...
	return (
		...
		>
			<title>{t(locale, "graph.curvePresetPreview")}</title>
			<path
				d={`M${points.join("L")}`}
				fill="none"
