...
import {
	getDbFromLinePos,
	getLinePosFromDb,
} from "@timeline/timeline/audio-display";
import { VOLUME_DB_MAX, VOLUME_DB_MIN } from "@timeline/timeline/audio-constants";
import { getElementVolume, hasAnimatedVolume } from "@timeline/timeline/audio-state";
...
export function AudioVolumeLine({
	...
	trackId: string;
	/** 同轨同步目标；传入时拖拽会把整组元素一起调到同一音量。 */
	syncElements?: AudioCapableElement[];
}) {
	...
	const previewVolume = useCallback(
		(nextVolume: number) => {
			...
			const targets =
				syncElements && syncElements.length > 0
					? syncElements
					: [element];
		...
		[element, editor, syncElements, trackId],
	);
		...
		<div className="pointer-events-none absolute inset-0">
			<div ref={surfaceRef} className="absolute inset-0">
				...
				<div
					...
					title={t(locale, "timeline.volumeHint")}
				/>
				{isDragging &&
					tooltipClientPos &&
					createPortal(
