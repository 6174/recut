import type { TimelineTrack, TimelineElement } from "@timeline/timeline";
import type { ComputeDropTargetParams, DropTarget } from "@timeline/timeline";
import { resolveTrackPlacement } from "@timeline/timeline/placement";
import { TIMELINE_TRACK_GAP_PX } from "./layout";
import { getTrackHeight } from "./track-layout";
import { getTimelineDisplayTracks } from "./track-layout";
import {
	mediaTime,
	type MediaTime,
	roundMediaTime,
...
export function computeDropTarget({
	...
	const orderedTracks = getTimelineDisplayTracks({ tracks });
	const fullTracks = [...tracks.overlay, tracks.main, ...tracks.audio];
	const toDisplayInsertIndex = (insertIndex: number): number =>
		fullTracks
			.slice(0, insertIndex)
			.filter((track) => orderedTracks.some(({ id }) => id === track.id)).length;
	const xPosition =
	...
	if (orderedTracks.length === 0) {
		...
		return {
			trackIndex: toDisplayInsertIndex(emptyTimelineResult.insertIndex),
			isNewTrack: true,
	...
	if (!trackAtMouse) {
		...
		strategy: {
			type: "preferIndex",
			trackIndex: isAboveAllTracks ? 0 : fullTracks.length - 1,
		...
		return {
			trackIndex: toDisplayInsertIndex(outOfBoundsResult.insertIndex),
			isNewTrack: true,
	...
	const fullTrackIndex = fullTracks.findIndex(({ id }) => id === track.id);
	...
	if (placementResult.kind === "existingTrack") {
		const displayTrackIndex = orderedTracks.findIndex(
			(track) => track.id === placementResult.trackId,
		);
	...
	return {
		trackIndex: toDisplayInsertIndex(placementResult.insertIndex),
		isNewTrack: true,
