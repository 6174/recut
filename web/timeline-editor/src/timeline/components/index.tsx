...
import { ScrollArea } from "@timeline/components/ui/scroll-area";
import {
	CaptionsIcon,
	Delete02Icon,
	MagicWand05Icon,
...
import { OcShapesIcon, OcVideoIcon } from "@timeline/components/icons";
import {
	ContextMenu,
	ContextMenuContent,
	ContextMenuItem,
...
import { useTimelineZoom } from "@timeline/timeline/hooks/use-timeline-zoom";
import {
	useCallback,
	useEffect,
	useMemo,
...
import type { ElementDragView, DropTarget } from "@timeline/timeline";
import { TimelineTrackContent } from "./timeline-track";
import { TimelinePlayhead } from "./timeline-playhead";
import { SelectionBox } from "@timeline/selection/selection-box";
import { useBoxSelect } from "@timeline/selection/hooks/use-box-select";
...
import { useElementInteraction } from "@timeline/timeline/hooks/element/use-element-interaction";
import { isCaptionTrack } from "@timeline/subtitles/shared-style";
...
import { timelineTimeToPixels } from "@timeline/timeline/pixel-utils";
import {
	getTrackHeight,
...
import { useTimelineSeek } from "@timeline/timeline/hooks/use-timeline-seek";
import { useTimelineDragDrop } from "@timeline/timeline/hooks/use-timeline-drag-drop";
import { TimelineRuler } from "./timeline-ruler";
import {
	TimelineBookmarksRow,
	useBookmarkDrag,
} from "@timeline/timeline/bookmarks/index";
import { useEdgeAutoScroll } from "@timeline/timeline/hooks/use-edge-auto-scroll";
import { useInitialScrollBottom } from "@timeline/timeline/hooks/use-initial-scroll-bottom";
import { useTimelineResize } from "@timeline/timeline/hooks/use-timeline-resize";
import { useTimelineStore } from "@timeline/timeline/timeline-store";
import { useEditor } from "@timeline/editor/use-editor";
import { useScrollPosition } from "@timeline/timeline/hooks/use-scroll-position";
import { useTimelinePlayhead } from "@timeline/timeline/hooks/use-timeline-playhead";
import { DragLine } from "./drag-line";
import { invokeAction } from "@timeline/actions";
...
export function Timeline() {
	...
	const tracks = useMemo<TimelineTrack[]>(
		() =>
			scene
				? getTimelineDisplayTracks({ tracks: scene.tracks })
				: [],
		[scene],
	);
			...
			{sortedTracks.map(({ track, index }) => (
				<ContextMenu key={track.id}>
					...
					<ContextMenuContent className="w-40">
						...
						>
							{t(locale, "timeline.pasteElements")}
						</ContextMenuItem>
						<ContextMenuItem
						...
						>
							{canTrackHaveAudio(track) && track.muted
								? t(locale, "timeline.unmuteTrack")
								: t(locale, "timeline.muteTrack")}
						</ContextMenuItem>
						<ContextMenuItem
						...
						>
							{canTrackBeHidden(track) && track.hidden
								? t(locale, "timeline.showTrack")
								: t(locale, "timeline.hideTrack")}
						</ContextMenuItem>
...
function TrackIcon({ track }: { track: TimelineTrack }) {
	if (isCaptionTrack(track)) {
		return (
			<HugeiconsIcon
				icon={CaptionsIcon}
				className="text-sky-400 size-4 shrink-0"
			/>
		);
	}
			...
			{rows.map((row) => (
				...
				>
					<span className="text-muted-foreground truncate text-xs leading-none">
						{getPropertyLabel({ path: row.propertyPath, locale })}
					</span>
				</div>
			))}
		</div>
