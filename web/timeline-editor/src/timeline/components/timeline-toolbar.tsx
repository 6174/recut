/**
 * [INPUT]: 依赖 EditorCore、时间线选择状态、场景状态与工具栏 UI 原语。
 * [OUTPUT]: 对外提供 TimelineToolbar 和场景、缩放、元素操作控件。
 * [POS]: timeline 的全局操作栏；重载期间只能读取可空场景状态。
...
import { TIMELINE_ZOOM_MAX } from "@timeline/timeline/scale";
import { sliderToZoom, zoomToSlider } from "@timeline/timeline/zoom-utils";
import { type TActionWithOptionalArgs, invokeAction } from "@timeline/actions";
...
import { hasMediaId } from "@timeline/timeline";
...
import { OcRippleIcon } from "@timeline/components/icons";
import { GraphEditorPopover } from "./graph-editor/popover";
import { PopoverTrigger } from "@timeline/components/ui/popover";
		...
		<div className="flex items-center gap-1">
			<TooltipProvider delayDuration={500}>
				<ToolbarButton
					...
					tooltip={t(locale, "timeline.split")}
					onClick={({ event }) => handleAction({ action: "split", event })}
				/>

				<ToolbarButton
					icon={<HugeiconsIcon icon={AlignLeftIcon} />}
					tooltip={t(locale, "timeline.splitLeft")}
					onClick={({ event }) => handleAction({ action: "split-left", event })}
				/>

				<ToolbarButton
					icon={<HugeiconsIcon icon={AlignRightIcon} />}
					tooltip={t(locale, "timeline.splitRight")}
					onClick={({ event }) =>
				...
				<ToolbarButton
					...
					tooltip={t(locale, "timeline.duplicate")}
					onClick={({ event }) =>
				...
				<ToolbarButton
					...
					tooltip={t(locale, "timeline.freezeFrame")}
					disabled={true}
				...
				<ToolbarButton
					...
					tooltip={t(locale, "timeline.deleteElement")}
					onClick={({ event }) =>
				...
				<Tooltip>
					<ToolbarButton
						...
						tooltip={isCurrentlyBookmarked ? t(locale, "timeline.removeBookmark") : t(locale, "timeline.addBookmark")}
						onClick={({ event }) =>
		...
		<div className="flex items-center gap-1">
			<TooltipProvider delayDuration={500}>
				<ToolbarButton
					...
					tooltip={t(locale, "timeline.autoSnapping")}
				...
				<ToolbarButton
					...
					tooltip={t(locale, "timeline.rippleEditing")}
