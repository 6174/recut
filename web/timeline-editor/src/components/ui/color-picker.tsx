/**
 * [INPUT]: 依赖颜色解析工具、Popover 与基础表单控件。
 * [OUTPUT]: 对外提供色块选择、取色器和可直接编辑 HEX/RGB/HSL/HSV 的 ColorPicker。
 * [POS]: components/ui 的颜色值唯一编辑入口，被属性、遮罩、书签和画布背景复用。
			...
			<header className="border-b flex justify-between items-center pb-2 px-2">
				<Select defaultValue="custom">
					<SelectTrigger variant="outline">
						<SelectValue placeholder={t(locale, "ui.selectMode")} />
					</SelectTrigger>
					<SelectContent position="popper">
						<SelectItem value="custom">{t(locale, "ui.custom")}</SelectItem>
						<SelectItem value="saved">{t(locale, "ui.saved")}</SelectItem>
					</SelectContent>
				</Select>
				<div>
		...
		return (
			<Popover>
				<div
					ref={ref}
					className={cn(
						"bg-accent/18 flex h-7 border flex-1 items-center gap-2 rounded-md px-[0.45rem]",
						className,
					)}
					{...props}
				>
					...
					<div className="flex flex-1 items-center">
						<span
							className="text-muted-foreground shrink-0 text-sm select-none"
							aria-hidden="true"
						>
							#
						</span>
						<Input
							className="border-0! bg-transparent px-1 py-0 ring-0! ring-offset-0! uppercase"
							size="sm"
							containerClassName="w-full"
							aria-label={t(locale, "ui.hexColor")}
							value={inputValue}
							onChange={handleInputChange}
							onBlur={handleInputBlur}
							onKeyDown={handleInputKeyDown}
