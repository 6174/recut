...
import { shouldMaintainPitch } from "@timeline/retime/rate";
import type { MediaAsset } from "@timeline/media/types";
import { applyAudioMasteringToBuffer } from "@timeline/media/audio-mastering";
import type { AudioCapableElement } from "@timeline/timeline/audio-state";
import {
	hasAnimatedVolume,
	isElementMuted,
	resolveEffectiveAudioGain,
} from "@timeline/timeline/audio-state";
import { doesElementHaveEnabledAudio } from "@timeline/timeline/audio-separation";
import { canElementHaveAudio, hasMediaId } from "@timeline/timeline/element-utils";
import { canTrackHaveAudio } from "@timeline/timeline";
import { mediaSupportsAudio } from "@timeline/media/media-utils";
import {
	buildDuckEnvelope,
	trackRoleOf,
	type DuckEnvelope,
} from "@timeline/timeline/duck";
import { getSourceTimeAtClipTime, renderRetimedBuffer } from "@timeline/retime";
import { Input, ALL_FORMATS, BlobSource, AudioBufferSink } from "mediabunny";
import { TICKS_PER_SECOND } from "@timeline/wasm";
import {
	computeRmsBuckets,
	type SampleBucket,
} from "@timeline/media/waveform-summary";
...
export interface CollectedAudioElement {
	...
	volume: number;
	muted: boolean;
	trackRole: "anchor" | "follower" | "none";
	retime?: RetimeConfig;
}
...
export async function collectAudioElements({
	...
	for (const { element, mediaAsset, trackRole } of candidates) {
...
export async function collectAudioClips({
	...
	for (const track of orderedTracks) {
		...
		const trackRole = trackRoleOf(track);
			...
			if (element.type === "audio") {
				if (element.sourceType === "upload") {
					...
					pendingLibraryClips.push(
						fetchLibraryAudioClip({ element, muted, volume, trackRole }),
					);
				}
				continue;
			}
...
export async function createTimelineAudioBuffer({
	...
	const duckEnvelope = buildDuckEnvelope(tracks);
...
function mixAudioChannels({
	...
	const duckFactorAt = (timelineSec: number): number =>
		element.trackRole === "follower" ? duckEnvelope.factorAt(timelineSec) : 1;
	...
	for (let channel = 0; channel < outputChannels; channel++) {
		...
		for (let i = 0; i < renderedLength; i++) {
			...
			outputData[outputIndex] +=
				...
				gain *
				duckFactorAt(startTime + clipTime);
		}
	}
}
