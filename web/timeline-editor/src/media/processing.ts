...
import { getMediaTypeFromFile } from "@timeline/media/media-utils";
import { formatStorageBytes } from "@timeline/services/storage/quota";
...
export async function processMediaAssets({
	...
	for (const file of fileArray) {
		...
		if (!fileType) {
			toast.error(t(getRecutLocale(), "media.unsupported", { name: file.name }));
			continue;
		}
		...
		if (!storageCheck.canStore) {
			toast.error(t(getRecutLocale(), "media.noStorageFor", { name: file.name }), {
		...
		try {
			if (fileType === "image") {
				...
				try {
					...
					if (!videoData.canDecode) {
						toast.error(t(getRecutLocale(), "media.cantPreview", { name: file.name }), {
					...
					toast.error(t(getRecutLocale(), "media.couldntProcess", { name: file.name }), {
						description: message,
					});
				}
			...
			toast.error(t(getRecutLocale(), "media.failedProcess", { name: file.name }));
