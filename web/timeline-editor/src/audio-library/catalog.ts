import { assetPath } from "@timeline/utils/base-path";
import type { AudioLibraryCatalog } from "@timeline/audio-library/types";
...
/**
 * 由音频库 id 解析稳定的 CDN URL。用于恢复时间线里失效的 blob sourceUrl
 * （OPFS 缓存是 origin 级的；跨 origin/清缓存后需回退到 CDN）。
 */
export async function resolveAudioLibraryCdnUrl(
	audioId: string,
): Promise<string | null> {
	try {
		const catalog = await loadAudioCatalog();
		const item = [...catalog.music, ...catalog.sfx].find(
			(entry) => entry.id === audioId,
		);
		return item ? audioAssetUrl(item.url) : null;
	} catch {
		return null;
	}
}
