import type { SceneTracks, TimelineElement } from "@timeline/timeline";
import type { MediaAsset } from "@timeline/media/types";
import { DEFAULT_GRAPHIC_SOURCE_SIZE } from "@timeline/graphics";
import { measureTextElement } from "@timeline/text/measure-element";
import { isComponentElementSelectable } from "@timeline/runtime";
import { getRenderedNodeBounds } from "@timeline/runtime/node-registry";
import {
	getElementLocalTime,
} from "@timeline/animation";
import { resolveTransformAtTime } from "@timeline/rendering/animation-values";
import { buildTransformFromParams } from "@timeline/rendering";

/**
 * [INPUT]: 时间线元素、媒体尺寸、组件注册表与运行时节点 bbox
 * [OUTPUT]: 可见元素的旋转 bounds，供预览选择、命中与变换控制器使用
 * [POS]: preview 的统一几何适配层；组件仅使用运行时已确认的真实渲染几何
...
function getElementBounds({
	...
	// D5：优先从已渲染的 Object3D 实时取 bbox（选择框 = 眼睛看到的几何）。
	// 仅当该元素已渲染（preview 挂载中）时可用；否则回退到 footprint 推算（导出/headless）。
	const rendered = getRenderedNodeBounds({
	...
	if (element.type === "component") {
		// 组件（尤其 html/react）不能根据承载面推测真实可见范围。未得到 runtime
		// 几何时宁可暂不显示控制框，也不能伪造一块全画布选区。
		return null;
	}
...
export function getEdgeHandlePosition({
	...
	const localY = edge === "top" ? -halfHeight : edge === "bottom" ? halfHeight : 0;
	return {
		x: bounds.cx + (localX * cos - localY * sin),
		y: bounds.cy + (localX * sin + localY * cos),
	};
...
export function getVisibleElementsWithBounds({
	...
	for (const track of orderedTracks) {
		const elements = track.elements
			...
			.filter((element) => isComponentElementSelectable(element))
			.filter(
				(element) =>
