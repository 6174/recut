...
import { isHtmlInCanvasSupported } from "@timeline/three/dom-text-surface";
import {
	getRenderedNodeBounds,
	getRenderedNodeObject,
} from "@timeline/runtime/node-registry";
import type { TimelineElement } from "@timeline/timeline";
import { buildComponentElement, buildTextElement } from "@timeline/timeline/element-utils";
import type { ParamValue } from "@timeline/params";
import { mediaTimeFromSeconds, mediaTimeToSeconds } from "@timeline/wasm";
...
export function installRecutTestBridge(): void {
	...
	(window as any).__recutTest = {
		...
		async renderExportProbe(timeSec = 0, options?: { onlyIds?: string[]; frames?: number[] }) {
			const { buildWorld, WorldRenderer, componentsRegistry } = await import("@timeline/runtime");
		...
		async runSceneExporterDirect() {
			const { SceneExporter } = await import("@timeline/services/renderer/scene-exporter");
			const { buildWorld } = await import("@timeline/runtime");
			const scene = editor.scenes.getActiveSceneOrNull();
			const project = editor.project.getActiveOrNull();
		...
		async loadFont(family: string) {
			const { loadFullFont } = await import("@timeline/fonts/google-fonts");
