...
/**
 * [INPUT]: 依赖时间线效果与素材面板 Store，依赖编辑器预览能力即时呈现参数变更。
 * [OUTPUT]: 对外提供片段效果属性面板与跳转至素材特效入口的空态。
 * [POS]: effects 的编辑侧属性界面；空态直接指向素材面板中已提升的特效顶层入口。
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */

import { useState } from "react";
import type { ParamValues } from "@timeline/params";
import type { Effect } from "@timeline/effects/types";
import type { EffectElement, VisualElement } from "@timeline/timeline";
import { effectsRegistry, getEffectName } from "@timeline/effects";
...
import { PropertyParamField } from "@timeline/components/editor/panels/properties/components/property-param-field";
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center">
				<SectionTitle>{t(locale, "prop.effects.title")}</SectionTitle>
			</div>
			<EffectSection
				effect={effect}
		...
		<div className="flex flex-col h-full items-center justify-center gap-4 text-center">
			...
			<div className="flex flex-col gap-2">
				<h3 className="font-medium text-foreground">
					{t(locale, "prop.effects.noEffects")}
				</h3>
				<p className="text-muted-foreground text-sm text-balance max-w-44">
					{t(locale, "prop.effects.noEffectsHint")}
				</p>
			</div>
			<Button
				variant="default"
					...
					onToggle && (
						<div className="flex items-center gap-1">
							<Button
								...
								size="icon"
								aria-label={t(locale, "prop.effects.toggle", { name })}
								onClick={onToggle}
							>
								<HugeiconsIcon
