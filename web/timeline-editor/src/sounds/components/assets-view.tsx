...
import { Separator } from "@timeline/components/ui/separator";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@timeline/components/ui/tabs";
import { useInfiniteScroll } from "@timeline/hooks/use-infinite-scroll";
import { useSoundSearch } from "@timeline/sounds/use-sound-search";
import { useSoundsStore } from "@timeline/sounds/sounds-store";
import type { SavedSound, SoundEffect } from "@timeline/sounds/types";
		...
		<div className="flex h-full flex-col">
			<Tabs defaultValue="sound-effects" className="flex h-full flex-col">
				<div className="px-3 pt-4 pb-0">
					<TabsList>
						<TabsTrigger value="sound-effects">{t(locale, "sounds.title")}</TabsTrigger>
						<TabsTrigger value="saved">{t(locale, "sounds.saved")}</TabsTrigger>
					</TabsList>
				</div>
		...
		const fetchTopSounds = async () => {
			try {
				...
				if (!shouldIgnore) {
					...
					setError({
						error:
							error instanceof Error ? error.message : t(locale, "sounds.failedLoad"),
					});
				}
			} finally {
				if (!shouldIgnore) {
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center gap-3">
				<Input
					placeholder={t(locale, "sounds.search")}
					className="w-full"
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="w-56">
						...
						>
							{t(locale, "sounds.commercialOnly")}
						</DropdownMenuCheckboxItem>
						<div className="text-muted-foreground px-2 py-1.5 text-xs">
							{showCommercialOnly
								? t(locale, "sounds.commercialOnlyActive")
								: t(locale, "sounds.commercialOnlyInactive")}
						</div>
					</DropdownMenuContent>
				</DropdownMenu>
			</div>
				...
				>
					<div className="flex flex-col gap-4">
						...
						{isSearching && searchQuery && (
							<div className="text-muted-foreground text-sm">{t(locale, "sounds.searching")}</div>
						)}
						...
						{!isLoading && !isSearching && displayedSounds.length === 0 && (
							<div className="text-muted-foreground text-sm">
								{searchQuery ? t(locale, "sounds.noneFound") : t(locale, "sounds.noneAvailable")}
							</div>
						)}
						{isLoadingMore && (
		...
		return (
			<div className="flex h-full items-center justify-center">
				<div className="text-destructive text-sm">
					{t(locale, "sounds.savedError", { error: savedSoundsError })}
				</div>
			</div>
		);
	}
		...
		return (
			<div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
				...
				<div className="flex flex-col gap-2 text-center">
					<p className="text-lg font-medium">{t(locale, "sounds.noSaved")}</p>
					<p className="text-muted-foreground text-sm text-balance">
						{t(locale, "sounds.noSavedHint")}
					</p>
				</div>
			</div>
		);
		...
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center justify-between">
				<p className="text-muted-foreground text-sm">
					{t(locale, "sounds.savedCount", { count: savedSounds.length })}
				</p>
				<Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
					<DialogTrigger asChild>
						...
						>
							{t(locale, "sounds.clearAll")}
						</Button>
					</DialogTrigger>
					<DialogContent>
						<DialogHeader>
							<DialogTitle>{t(locale, "sounds.clearAllTitle")}</DialogTitle>
							<DialogDescription>
								{t(locale, "sounds.clearAllConfirm", {
						...
						<DialogFooter>
							...
							>
								{t(locale, "sounds.clearAllSounds")}
							</Button>
						</DialogFooter>
					</DialogContent>
				</Dialog>
		...
		<div className="group flex items-center gap-3 opacity-100 hover:opacity-75">
			...
			<div className="flex items-center gap-3 pr-2">
				<Button
					...
					title={t(locale, "sounds.addToTimeline")}
				>
				...
				<Button
					...
					title={isSaved ? t(locale, "sounds.removeSaved") : t(locale, "sounds.saveSound")}
				>
					<HugeiconsIcon
						icon={FavouriteIcon}
