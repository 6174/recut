...
import type { SoundEffect, SavedSound } from "@timeline/sounds/types";
...
import { buildLibraryAudioElement } from "@timeline/timeline/element-utils";
...
export const useSoundsStore = create<SoundsStore>((set, get) => ({
	...
	saveSoundEffect: async ({ soundEffect }) => {
		try {
			...
			toast.error(t(getRecutLocale(), "sounds.failedSave"));
	...
	removeSavedSound: async ({ soundId }) => {
		try {
			...
			set({ savedSoundsError: errorMessage });
			toast.error(t(getRecutLocale(), "sounds.failedRemove"));
	...
	clearSavedSounds: async () => {
		try {
			...
			set({ savedSoundsError: errorMessage });
			toast.error(t(getRecutLocale(), "sounds.failedClear"));
	...
	addSoundToTimeline: async ({ sound }) => {
		...
		if (!audioUrl) {
			toast.error(t(getRecutLocale(), "sounds.notAvailable"));
			return false;
		}
		...
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: t(getRecutLocale(), "sounds.failedAdd"),
				{ id: `sound-${sound.id}` },
			);
			return false;
		}
