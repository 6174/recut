/**
 * [INPUT]: 依赖遮罩注册表、时间线预览和全局表单控件。
 * [OUTPUT]: 对外提供 MasksTab，统一编辑遮罩的数值、颜色、文本与几何属性。
 * [POS]: masks/components 的主属性面板，遵循编辑器单行字段与范围控件标准。
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */
"use client";

import type { MaskableElement } from "@timeline/timeline";
import type { Mask, MaskType, TextMask } from "@timeline/masks/types";
import type {
	NumberParamDefinition,
	SelectParamDefinition,
} from "@timeline/params";
import { getParamLabel, getSelectOptionLabel } from "@timeline/params";
import {
...
import { useMenuPreview } from "@timeline/editor/use-menu-preview";
...
import { ColorPicker } from "@timeline/components/ui/color-picker";
import { FontPicker } from "@timeline/components/ui/font-picker";
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
	DropdownMenuTrigger,
} from "@timeline/components/ui/dropdown-menu";
import { NumberField } from "@timeline/components/ui/number-field";
import { Slider } from "@timeline/components/ui/slider";
import {
	Select,
	SelectContent,
	SelectItem,
	SelectTrigger,
	SelectValue,
} from "@timeline/components/ui/select";
import { Textarea } from "@timeline/components/ui/textarea";
import {
	Tooltip,
	TooltipContent,
	TooltipTrigger,
} from "@timeline/components/ui/tooltip";
import {
	clamp,
	formatNumberForDisplay,
	getFractionDigitsForStep,
	snapToStep,
} from "@timeline/utils/math";
import {
	Section,
	SectionContent,
	SectionField,
	...
	SectionHeader,
	SectionTitle,
} from "@timeline/components/section";
import { usePropertyDraft } from "@timeline/components/editor/panels/properties/hooks/use-property-draft";
import {
	OcMirrorIcon,
	OcShapesIcon,
	OcTextHeightIcon,
	OcTextWidthIcon,
} from "@timeline/components/icons";
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center justify-between gap-2">
				<SectionTitle>{t(locale, "prop.masks.title")}</SectionTitle>
				<DropdownMenu
					...
					{hasMask ? (
						<Tooltip>
							<TooltipTrigger asChild>
								<span className="inline-flex">
									<Button
										...
										size="icon"
										disabled
										aria-label={t(locale, "prop.masks.add")}
									>
							...
							<TooltipContent className="max-w-56 text-balance">
								{t(locale, "prop.masks.singleMaskHint")}
							</TooltipContent>
						</Tooltip>
					) : (
...
function MaskItem({
	...
	const name = getMaskName({ definition, locale });

	return (
		<Section collapsible sectionKey={`mask-item:${mask.id}`} showTopBorder={false}>
			<SectionHeader
				trailing={
					<div className="flex items-center gap-1">
						<Button
							variant="ghost"
							size="icon"
							aria-label={t(locale, "prop.masks.toggleInversion", { name })}
							onClick={() =>
						...
						<Button
							variant="ghost"
							size="icon"
							aria-label={t(locale, "prop.masks.remove", { name })}
							onClick={() =>
			...
			>
				<div className="flex items-center gap-2">
					...
					<SectionTitle className="capitalize font-normal">{name}</SectionTitle>
				</div>
			</SectionHeader>
			<SectionContent>
				<MaskParamsFields
		...
		<SectionFields>
			...
			{definition.features.hasPosition &&
				...
				"centerY" in mask.params && (
					<SectionField label={t(locale, "prop.masks.position")}>
						<div className="flex items-center gap-2">
							<MaskNumberField
								className="flex-1"
								icon="X"
			...
			{definition.features.sizeMode === "width-height" &&
				...
				"height" in mask.params && (
					<SectionField label={t(locale, "prop.masks.size")}>
						<div className="flex items-center gap-2">
							<MaskNumberField
								className="flex-1"
								icon="W"
			...
			{definition.features.sizeMode === "height-only" &&
				"height" in mask.params && (
					<SectionField label={t(locale, "prop.masks.height")}>
						<MaskNumberField
							icon="H"
							param={getNumberParamDefinition({
								definition,
			...
			{definition.features.sizeMode === "width-only" &&
				"width" in mask.params && (
					<SectionField label={t(locale, "prop.masks.width")}>
						<MaskNumberField
							icon="W"
							param={getNumberParamDefinition({
								definition,
			...
			{definition.features.sizeMode === "uniform" && "scale" in mask.params && (
				<SectionField label={t(locale, "prop.masks.scale")}>
					<MaskNumberField
						icon={
			...
			{definition.features.hasRotation && "rotation" in mask.params && (
				<SectionField label={t(locale, "prop.masks.rotation")}>
					<MaskNumberField
			...
			<SectionField label={t(locale, "prop.masks.feather")}>
				<MaskNumberField
			...
			<SectionField label={t(locale, "prop.masks.stroke")}>
...
const LETTER_SPACING_PARAM: NumberParamDefinition = {
	...
	labelKey: "prop.masks.letterSpacing",
	type: "number",
	default: 0,
	min: -100,
	max: 500,
...
const LINE_HEIGHT_PARAM: NumberParamDefinition = {
	key: "lineHeight",
	label: "Line height",
	labelKey: "prop.masks.lineHeight",
	type: "number",
	default: 1.2,
	min: 0.1,
	max: 10,
...
function TextMaskFields({
	...
	return (
		<>
			<SectionField label={t(locale, "prop.masks.content")}>
				<Textarea
			...
			<SectionField label={t(locale, "prop.masks.font")}>
				<FontPicker
			...
			<SectionField label={t(locale, "prop.masks.spacing")}>
...
function MaskNumberField({
	...
	const previewFromDisplay = (nextDisplayValue: number) => {
		const clamped = clampDisplay(
			snapToStep({ value: nextDisplayValue, step }),
		);
		...
		<div className={cn("flex min-w-0 items-center gap-2", className)}>
			...
			{isRange && (
				<Slider
					...
					value={[clampDisplay(effectiveDisplayValue)]}
					min={min}
					max={max}
					step={step}
