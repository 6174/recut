/**
 * [INPUT]: EditorCore 播放状态、时间线音频轨、MediaBunny Input/AudioBufferSink。
 * [OUTPUT]: AudioManager；调度音频片段、处理播放会话与可取消的解码迭代器。
 * [POS]: core/managers 的音频播放协调器；预览播放结束时吞掉已释放 Input 的竞态错误。
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */
import type { EditorCore } from "@timeline/core";
import { TICKS_PER_SECOND } from "@timeline/wasm";
import { clampRetimeRate, shouldMaintainPitch } from "@timeline/retime/rate";
import type { AudioClipSource } from "@timeline/media/audio";
import { createAudioContext, collectAudioClips } from "@timeline/media/audio";
import {
	buildAudioGainAutomation,
	hasAnimatedVolume,
} from "@timeline/timeline/audio-state";
import { buildDuckEnvelope, type DuckEnvelope } from "@timeline/timeline/duck";
import { createAudioMasteringChain } from "@timeline/media/audio-mastering";
import {
	getClipTimeAtSourceTime,
	getSourceTimeAtClipTime,
	renderRetimedBuffer,
} from "@timeline/retime";
import {
	ALL_FORMATS,
	AudioBufferSink,
	BlobSource,
...
function isInputDisposedError(error: unknown): boolean {
	if (!(error instanceof Error)) return false;
	return error.name === "InputDisposedError" || error.message.includes("Input has been disposed");
}
...
export class AudioManager {
	...
	private async startPlayback({ time }: { time: number }): Promise<void> {
		...
		this.duckEnvelope = buildDuckEnvelope(tracks);
		if (!this.editor.playback.getIsPlaying()) return;
		...
		for (const clip of this.clips) {
			...
			if (this.shouldUsePreparedClipBuffer({ clip })) {
				...
				void this.runClipIterator({
					...
					startTime: currentTime,
					sessionId: this.playbackSessionId,
				}).catch(this.handleClipIteratorError);
			}
		}
	}
	...
	private handleClipIteratorError = (error: unknown): void => {
		if (isInputDisposedError(error) || !this.editor.playback.getIsPlaying()) return;
		console.error("Audio clip iterator failed:", error);
	};
	...
	private async runClipIterator({
		...
		for await (const { buffer, timestamp } of iterator) {
			...
			clipGain.gain.value =
				clip.volume *
				this.duckFactorAt({ clip, timelineTime });
	...
	private duckFactorAt({
		...
		if (clip.trackRole !== "follower" || !this.duckEnvelope) return 1;
		return this.duckEnvelope.factorAt(timelineTime);
	}
	...
	private scheduleClipGainAutomation({
		...
		clipGain.gain.setValueAtTime(
			clip.volume * this.duckFactorAt({ clip, timelineTime: clip.startTime + startLocalTime }),
			startTimestamp,
		);
		...
		clipGain.gain.setValueAtTime(
			points[0].gain * this.duckFactorAt({ clip, timelineTime: clip.startTime + points[0].localTime }),
			startTimestamp,
		);
		for (let index = 1; index < points.length; index++) {
			...
			clipGain.gain.linearRampToValueAtTime(
				point.gain * this.duckFactorAt({ clip, timelineTime: clip.startTime + point.localTime }),
				pointTimestamp,
			);
		}
	}
