...
import type {
	...
	TProjectSettings,
	TTimelineViewState,
} from "@timeline/project/types";
import type { ExportOptions, ExportResult, ExportState } from "@timeline/export";
import {
	normalizeRecutProject,
	storageService,
} from "@timeline/services/storage/service";
import { toast } from "sonner";
import { t, getRecutLocale } from "@timeline/i18n";
import { generateUUID } from "@timeline/utils/id";
import { UpdateProjectSettingsCommand } from "@timeline/commands/project";
import { DEFAULT_BACKGROUND_COLOR } from "@timeline/background/color";
import { DEFAULT_CANVAS_SIZE } from "@timeline/canvas/sizes";
import { DEFAULT_FPS } from "@timeline/fps/defaults";
import { buildDefaultScene, getProjectDurationFromScenes } from "@timeline/timeline/scenes";
import { WorldRenderer } from "@timeline/runtime";
import { buildWorld } from "@timeline/runtime/build-world";
import { mediaTimeToSeconds } from "@timeline/wasm";
import {
...
import { loadFontsBySource } from "@timeline/fonts/google-fonts";
import { syncTimelineComponents } from "@timeline/recut/components";
import { restoreLibraryAudioSourceUrls } from "@timeline/audio-library/restore";
import { resolveAudioLibraryCdnUrl } from "@timeline/audio-library/catalog";
import { DEFAULTS } from "@timeline/timeline/defaults";
import { getElementFontFamilies } from "@timeline/timeline/element-utils";
import { getRaisedProjectFpsForImportedMedia } from "@timeline/fps/utils";
...
export class ProjectManager {
	...
	async loadProject({ id }: { id: string }): Promise<void> {
		...
		try {
			...
			// 恢复内部缓存音频（音频库下载）的可播放 sourceUrl；OPFS 是 origin 级的，
			// 跨 origin/清缓存后回退到音频库 CDN URL，避免保留失效的 blob URL。
			await restoreLibraryAudioSourceUrls({
				scenes: project.scenes ?? [],
				resolveCdnUrl: resolveAudioLibraryCdnUrl,
			});
	...
	private async loadProjectFonts({
		...
		const { loadFontCatalog } = await import("@timeline/fonts/service-catalog");
	...
	private buildFirstFrameWorld(): import("@timeline/runtime/types").World | null {
		...
		const world = buildWorld({
			...
			duration: mediaTimeToSeconds({
				time: duration || (1 as import("@timeline/wasm").MediaTime),
			}),
			background,
		});
