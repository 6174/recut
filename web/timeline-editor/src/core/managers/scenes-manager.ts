/**
 * [INPUT]: 依赖 EditorCore、存储服务与场景命令，管理场景生命周期和当前场景。
 * [OUTPUT]: 对外提供 ScenesManager，负责加载、切换、创建、删除与书签操作。
 * [POS]: core/managers 的场景状态边界；管理场景加载、切换与生命周期。
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */
import type { EditorCore } from "@timeline/core";
import type { Bookmark, SceneTracks, TScene } from "@timeline/timeline";
...
export class ScenesManager {
	...
	initializeScenes({
		...
		const didNormalizeTracks = ensuredScenes.some(
			(scene, index) => scene !== scenes[index],
		);
		const hasAddedMainScene = ensuredScenes.length > scenes.length;
		if (hasAddedMainScene || didNormalizeTracks) {			const activeProject = this.editor.project.getActive();
