...
import { Switch } from "@timeline/components/ui/switch";
import { HugeiconsIcon } from "@hugeicons/react";
import { DashboardSpeed02Icon } from "@hugeicons/core-free-icons";
import { buildConstantRetime } from "@timeline/retime";
import {
	DEFAULT_RETIME_RATE,
	MIN_RETIME_RATE,
	MAX_RETIME_RATE,
	clampRetimeRate,
	canMaintainPitch,
} from "@timeline/retime/rate";
import type { AudioElement, VideoElement } from "@timeline/timeline";
import {
	Section,
	SectionContent,
	SectionField,
...
export function SpeedTab({
	...
	return (
		<Section collapsible sectionKey={`${element.id}:speed`}>
			<SectionHeader>
				<SectionTitle>{t(locale, "prop.speed.title")}</SectionTitle>
			</SectionHeader>
			<SectionContent>
				<SectionFields>
					<SectionField label={t(locale, "prop.speed.title")}>
						<NumberField
					...
					<div className="flex items-center justify-between">
						<span className="text-sm">{t(locale, "prop.speed.changePitch")}</span>
						<Switch
