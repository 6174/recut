...
ComfyUI 工作台界面（React + TypeScript + Vite + Tailwind CSS v4）。
...
- `src/components/WorkflowTab.tsx`：工作流切换（shadcn Select）+ 未就绪时置于表单上方的核心依赖块（准备环境 / 下载模型 / 来源）+ 按 `formSchema` 渲染的通用表单（含 `media` 参考图，订阅 `useGenerateStore`）。
