...
调用平台媒体生成前检查能力快照的 `media.readiness.speech`（内建会话见 guide 文末「动态配置」，不必调 `recut.context`）。**先落位、不空等**：提交拿到 `assetId` 立即 `timeline.placeAudio` 落轨并标记生成中，平台就绪后自动切换；只有下一步依赖产物内容（试听验收、据人声再对齐）时才 `recut.job.wait` 到 `completed`，`failed` 如实报错。语音生成需要 `voiceId`，先从 `recut.media.list_voices` 获取真实 ID，不凭记忆捏造音色。
