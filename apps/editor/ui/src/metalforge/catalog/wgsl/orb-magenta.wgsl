// Magenta — the sheet's 01 · Reference, the baseline liquid orb.
//
...
// with a disc feather.
//
// Transcribed from the Claude Design sheet "Liquid Orbs", screen 01
// (Reference), on its CFG[0]. Every constant is the sheet's — the hash pair
// (127.1, 311.7) and its 34.56, the four octaves at gain 0.5 with the
// (1.6,1.2,-1.2,1.6) inter-octave rotation and NO weight-sum normalisation, the
// 1.05/0.85/2.7/3.1 warp field scales, the 0.55+0.9 and 0.45+0.9 amplitudes,
// the 0.020/0.014/0.010 wobble lobes, the 0.045 shade, the 0.40 glow offset and
// its 2.6 falloff, the 0.05 dispersion step, the 0.006 rim aberration, the
// pow-48 specular, the 0.85/2.4 halo. Each parameter's default IS the constant
// it replaced: `contrast` is CFG[0].cont, `flow` is its flow, `light` is its
// light. `radius` (0.86) is the sheet's own R0. Only `speed`, `grain` and
// `exposure` are new, and all three default to 1, where they are the identity.
//
// One idea, three files: orb-magenta.metal (what an iOS user exports), this one
// (what the browser preview runs) and orb-magenta.sksl (the React Native half).
...
//     it is transcribed verbatim. It is why a #F25AED stop paints as #F200ED:
...
// The sheet's per-orb constants that stayed constants. `LQO_LOOP` is CFG[0]'s
...
// The sheet's `pal(x)`, unrolled for this orb's TWO stops. With `uNC == 2` the
// general form's segment index is always 0 and its parameter is `x` itself, so
// the whole function is one smoothstepped mix plus the per-segment chroma boost
// `1 + 0.5*f*(1-f)` that gives the midtones their bite.
...
fn orbMagentaAnim(uv01: vec2<f32>) -> vec4<f32> {
  ...
  // The palette's stops and this orb's own two, both in hand. The stops stay in
  // sRGB and are converted after the ramp reads them, which costs three
