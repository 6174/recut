// Ember Orb — the sheet's 06 · Ember, the molten one. Named with the suffix
// because "Ember" alone is the gradient card; "Smoke Orb", "Nebula Orb" and
// "Abyss Orb" are the precedent.
//
// The silhouette holds still while the liquid inside churns: every point rides
// its own small circle once per loop, so the loop closes with no jump. Two
// warp fields (a slow one on the loop's own angle, a finer one running the
// other way) displace a marbled four-octave value-noise field; the scalar that
// comes out is graded through an Oklab ramp, dispersed per channel at the rim,
// rotated in hue by a shimmer term, lit by a rim light and a glint, and closed
// with a disc feather.
//
// Transcribed from the Claude Design sheet "Liquid Orbs", screen 06 (Ember), on
// its CFG[5]. Motion engine 0 — the same body as the sheet's reference screen,
// unchanged but for the palette arity below. Every constant is the sheet's —
// the hash pair (127.1, 311.7) and its 34.56, the four octaves at gain 0.5 with
// the (1.6,1.2,-1.2,1.6) inter-octave rotation and NO weight-sum normalisation,
// the 1.05/0.85/2.7/3.1 warp field scales, the 0.55+0.9 and 0.45+0.9
// amplitudes, the 0.020/0.014/0.010 wobble lobes, the 0.045 shade, the 0.40
// glow offset and its 2.6 falloff, the 0.05 dispersion step, the 0.006 rim
// aberration, the pow-48 specular, the 0.85/2.4 halo. Each parameter's default
// IS the constant it replaced: `contrast` is CFG[5].cont, `flow` is its flow,
// `light` is its light. `radius` (0.86) is the sheet's own R0. Only `speed`,
// `grain` and `exposure` are new, and all three default to 1, where they are
// the identity.
//
// WHAT MAKES THIS ONE READ AS MOLTEN RATHER THAN AS A COLD BALL — two things,
// and both are firsts on this shelf's liquid orbs:
//
//  * The halo is LIVE. `halo` defaults to 0.2 (CFG[5].halo), so the ball sits in
//    a ring of its own char colour lifted and desaturated, the way something hot
//    sits in the air around it, rather than ending at its own edge.
//  * It is lit from the LOWER RIGHT. `light` is (0.62, -0.7) — the only one of
//    these with a positive x — so the glint, the inner glow's offset and the
//    shade all come from below and to the side, which is where a fire is.
//
// THREE stops rather than two: Char, Flame, Ash. `lqoPal` is unrolled for that
// arity below, so the mid stop is a real segment boundary and the chroma boost
// runs twice across the ramp instead of once.
//
// One idea, three files: orb-ember.metal (what an iOS user exports), this one
// (what the browser preview runs) and orb-ember.sksl (the React Native half).
...
//     it is transcribed verbatim. It is why a #3D0E02 stop paints as #3D0802 and
//     a #FFB199 one as #FF9399: the green channel is crushed and the ball is
//     hotter than its swatch. "Fixing" it would make this port stop matching the
//     design.
...
// The sheet's per-orb constants that stayed constants. `LQO_LOOP` is CFG[5]'s
...
// Grain slider multiplies exactly as the sheet's own `grain` prop did — and at
// 0.08 it is the coarsest of these five, which is half of why this one reads as
// hot ash rather than as clean glass.
const LQO_LOOP:  f32 = 6.5;
const LQO_SOFT:  f32 = 1.3;
const LQO_GRAIN: f32 = 0.08;
...
// general form's `n` is 2, so the parameter runs 0..2, the segment index is
// `min(floor(s), 1)` — Char→Flame below the midpoint, Flame→Ash above it — and
// the per-segment chroma boost `1 + 0.5*f*(1-f)` runs once per segment rather
// than once across the whole ramp. SELECTED, never branched, for the reason the
// colour line below carries.
...
fn orbEmberAnim(uv01: vec2<f32>) -> vec4<f32> {
  ...
  // On this orb the band is not decorative — `halo` ships at 0.2.
  ...
  // This is the first of these liquid orbs to ship with it ON: at 0.2 the char
  // reads as heat standing off the ball rather than as a second edge.
