// Drop — the sheet's 02 · Drop, the one whose silhouette breathes.
//
// The sheet's own caption: "Three waves — 3, 5 and 7 lobes — travel the rim at
// different speeds while the core holds still. Glassy, high refraction."
//
// Same engine as 01 · Reference (orb-magenta) and the same body, line for line;
// what makes this screen its own effect is that it is the one of the five with
// `wob = 1`. The three rim lobes are therefore VISIBLE — 3, 5 and 7 lobes at
// three speeds and two directions, riding on the disc's radius while the liquid
// inside ignores them — and the ball reads as a hanging drop rather than as a
// sphere. Refraction is nearly doubled (0.85 against the Reference's 0.55), so
// the field is pulled hard toward the centre as the sphere turns away and the
// teal sits deep behind glass; the glint is hotter and the fringes are quieter.
//
// Underneath: every point rides its own small circle once per loop, so the loop
// closes with no jump. Two warp fields (a slow one on the loop's own angle, a
// finer one running the other way) displace a marbled four-octave value-noise
// field; the scalar that comes out is graded through an Oklab ramp, dispersed
// per channel at the rim, rotated in hue by a shimmer term, lit by a rim light
// and a glint, and closed with a disc feather.
//
// Transcribed from the Claude Design sheet "Liquid Orbs", screen 02 (Drop), on
// its CFG[1]. Every constant is the sheet's — the hash pair (127.1, 311.7) and
// its 34.56, the four octaves at gain 0.5 with the (1.6,1.2,-1.2,1.6)
// inter-octave rotation and NO weight-sum normalisation, the 1.05/0.85/2.7/3.1
// warp field scales, the 0.55+0.9 and 0.45+0.9 amplitudes, the 0.020/0.014/0.010
...
// `contrast` is CFG[1].cont, `flow` is its flow, `wobble` is its wob, `light` is
// its light. `radius` (0.86) is the sheet's own R0. Only `speed`, `grain` and
// `exposure` are new, and all three default to 1, where they are the identity.
//
// One idea, three files: orb-drop.metal (what an iOS user exports), this one
// (what the browser preview runs) and orb-drop.sksl (the React Native half).
// Same maths in three spellings — change one and change all three, or the Code
// tab starts lying about what it ships.
//
// THREE STOPS, NOT TWO. CFG[1]'s palette is teal → foam → glass, so the sheet's
// `pal()` runs at `uNC == 3`: two segments, a segment index of 0 or 1, and the
// chroma boost `1 + 0.5*f*(1-f)` applied per segment rather than once across the
// whole ramp. `lqoPal` below is that general form unrolled for exactly three,
// which is why it takes an extra colour and why the boost pinches twice.
//
...
//     it is transcribed verbatim. It is why a #0E9AA7 stop paints as #0E7CA7:
//     the green channel is crushed, and this orb's teal reads a shade bluer than
//     its swatch. "Fixing" it would make this port stop matching the design.
...
//
// Deliberately NOT ported from the sheet: the other nineteen CFG entries and
// the five alternate motion engines behind `uMode` (they would ship as dead
// shaders in one file — screen 19 is its own effect, orb-lava-lamp); the
// per-tile clock offset `0.17 * i`, which is a phase and not a difference in
// maths; and the sheet's own `uSoftPx`, folded into the constant below because
// the Orbs family already carries an `Edge softness` and two sliders for one
// feather is one too many.
//
...
// The sheet's per-orb constants that stayed constants. `LQO_LOOP` is CFG[1]'s
...
const LQO_LOOP:  f32 = 8.0;
const LQO_SOFT:  f32 = 1.6;
const LQO_GRAIN: f32 = 0.045;
...
// general form's `n` is 2, so `x` is stretched over two segments, the index is
// `min(floor(s), 1)` and the per-segment parameter `ff` is what gets
// smoothstepped — the boost `1 + 0.5*f*(1-f)` therefore pinches once per
// segment, at x = 0.25 and x = 0.75, and returns to 1 at the middle stop.
// SELECTED, never branched, for the reason the colour line below gives.
...
fn orbDropAnim(uv01: vec2<f32>) -> vec4<f32> {
  ...
  // uniform-only expression — no per-pixel value enters it. On THIS orb that
  // term is live rather than folded away, because Wobble defaults to 1: the
  // guard has to clear the fattest the silhouette ever gets, not its mean.
  //
  ...
  // and in different directions while the core holds still. This is the screen
  // it was written for — at Wobble 1 the drop breathes; at 0 it is the
  // Reference's still sphere.
  ...
  // away, which is what makes the liquid read as being INSIDE glass. At 0.85
  // this orb is the glassiest of the five.
