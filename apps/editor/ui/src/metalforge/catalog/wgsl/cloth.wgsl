// cloth.wgsl — the live WebGPU preview of the Cloth effect.
//
// A transcription of effects/cloth/cloth.metal, which is itself the ClothLab
// source app's shader verbatim. Same constants, same lighting model (Lambert +
// GGX, Charlie sheen with Neubelt visibility, Belcour/Barla thin-film
// iridescence, the analytic studio room, ACES filmic), same grain. Change one,
// change both.
//
// Four entry pairs, matching the four Metal pipelines:
//   vs_full / fs_background   the backdrop
//   vs_surface / fs_surface   the sheet
//   vs_points  / fs_points    the point cloud
//   vs_full / fs_composite    the film grain over the top
//
// One thing genuinely differs, because WebGPU has no sized point primitive:
// `cloth_points_vertex` sets `[[point_size]]` and Metal expands the sprite for
// us, while here each point is an instanced 4-vertex quad expanded around the
// projected centre by the same pixel size. `point_coord` becomes the quad's own
// local coordinate, and `length(local)` is exactly the `length(pc - 0.5) * 2`
// the MSL computes.
...
struct U {
  model: mat4x4<f32>,
  viewProj: mat4x4<f32>,
  camera: vec4<f32>,   // xyz camera position (world), w exposure
  base: vec4<f32>,     // xyz base colour (linear), w roughness
  sheenC: vec4<f32>,   // xyz sheen colour * sheen, w sheen amount
  tint: vec4<f32>,     // xyz image tint (linear), w image emissive
  pointC: vec4<f32>,   // xyz point colour (linear), w point opacity
  m0: vec4<f32>,       // metalness, sheenRoughness, envIntensity, bumpScale
  m1: vec4<f32>,       // iridescence, iridescenceIOR, thicknessMin, thicknessMax
  m2: vec4<f32>,       // hasBump, hasFill, isPicture, useLights
  tiles: vec4<f32>,    // bumpTile.xy, fillTile.xy
  env: vec4<f32>,      // useEnv, envBlur, time, pointSize
  view: vec4<f32>,     // viewportW, viewportH, pointRound, pointTakesFillColour
  grain: vec4<f32>,    // overlay, screen, scale, fixed point size in px (0 = attenuate)
  bg0: vec4<f32>,      // inner colour, gamma-encoded sRGB
  bg1: vec4<f32>,      // mid
  bg2: vec4<f32>,      // outer
  bgP: vec4<f32>,      // centre.xy, extent.xy
  bgM: vec4<f32>,      // mode (0 clear, 1 solid, 2 radial), aspect, unused, unused
};
...
// Belcour / Barla, as implemented in three.js.
...
fn vs_points(@builtin(vertex_index) vi: u32,
  ...
  // matches three's PointsMaterial sizeAttenuation: size * (viewportHeight/2) / -viewZ
  let viewZ = max(clip.w, 1e-3);
  var px = max(1.0, u.env.w * 0.5 * u.view.y / viewZ);
  if (u.grain.w > 0.0) { px = u.grain.w; }
  // A pixel is 2/viewport in NDC, so a sprite of diameter `px` has a half
  // extent of px/viewport. Scaled by w because this is still clip space.
...
fn fs_surface(in: SurfaceOut, @builtin(front_facing) frontFacing: bool) -> @location(0) vec4<f32> {
  ...
  // ---- fill: colour, gradient or picture -------------------------------
  ...
  // ---- weave bump ------------------------------------------------------
  // A fixed texel step and a cotangent frame, not three.js's screen-space
  // height derivatives — see the note in cloth.metal.
  ...
  // ---- unlit: the colour, and nothing over it --------------------------
  // Nothing lights this sheet — no room, no lamps — so it is shown as it is.
  // The fill is scaled by how far the surface has turned away from the viewer
  // and written straight out, which makes every pixel a SCALAR multiple of the
  // colour: the hue and the saturation are exactly the ones picked, and only
  // the brightness carries the fold. Facing you, the pixel IS that colour —
  // `linearToSRGB` undoes the linearisation the uniform packer did, and the
  // exposure multiply is the identity at its default of 1.
  //
  // This is not a mode flag; it is the absence of both light sources, which the
  // exported app expresses with the same two booleans on `ClothScene`.
  if (u.m2.w < 0.5 && u.env.x < 0.5) {
    var shade = 0.25 + 0.75 * sat(dot(normal, viewDir));
    // A picture wants to stay legible in a fold, which is what the source's
    // "image ready" emissive did; here it lifts the shading toward flat rather
    // than adding a second light.
  ...
  // curvature-driven roughness, so tight folds do not sparkle
  ...
  // ---- three directional lights + ambient ------------------------------
  if (u.m2.w > 0.5) {
    ...
    var LCOL = array<vec3<f32>, 3>(vec3<f32>(2.1, 2.1, 2.1),
                                   vec3<f32>(0.5085, 0.5820, 0.6841),   // 0xbfc8d8 * 0.65, linear
...
fn fs_points(in: PointOut) -> @location(0) vec4<f32> {
  // Sampled before the round mask can discard: `textureSample` wants uniform
  // control flow, and a discarded invocation is no longer one.
  ...
  // Straight alpha, as in the MSL — the pipeline blends src-alpha /
  // one-minus-src-alpha, matching ClothGPU's points pipeline.
