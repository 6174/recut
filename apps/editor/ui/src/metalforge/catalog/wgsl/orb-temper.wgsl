// Temper — a bismuth hopper crystal with the oxide sequence dragged across it,
// standing in a wall-only studio void (no floor, no contact shadow).
//
// Field order mirrors `mslArgOrder` in config.ts so the runner's generic uniform
// packer can walk one ordering. The ten floats sit back to back and WGSL then
// pads to put `tintColor` on a 16-byte boundary — the packer mirrors the gap.
// The sixteen colours then run back to back, the five originals first, the ten
// sweep additions after them and `filmColor` last.
struct Uniforms {
  size:          vec2<f32>,
  time:          f32,
  speed:         f32,
...
// The one rotation the crystal has — the terrace field turns with it, the
// silhouette does not.
...
// The room: a graded back wall with two soft key spills, and nothing below it —
// the ball stands in a void, with no floor, contact shadow or reflection.
// `wallA`/`wallB` are the two ends of the grade; `lamp` and `fill` are the
// spills. All four arrive already scaled by the caller.
...
// What a mirror facet sees: the room re-sampled along the reflected direction,
// three specular lobes off one key light, an iridescent band across the
// reflection and a warm bounce from below.
...
// The terrace field: two fbm octave stacks, the coarse one carrying the plateau
// count and the fine one roughening every riser.
...
fn orbTemperAnim(uv01: vec2<f32>) -> vec4<f32> {
  ...
  // The room palette. These five are near black, so each swatch holds the hue
  // normalised to its own maximum channel and the literal beside it restores
  // the level. The comment on each line is the constant it replaced, and the
  // default swatch reproduces it to well under 1/255.
  let wallA = u.wallColor.rgb     * 0.0112; // vec3<f32>(0.0080, 0.0086, 0.0112)
  let wallB = u.wallTintColor.rgb * 0.0027; // vec3<f32>(0.0016, 0.0016, 0.0027)
  let lamp  = u.lampColor.rgb     * 0.0270; // vec3<f32>(0.0210, 0.0220, 0.0270)
  let fill  = u.fillColor.rgb     * 0.0135; // vec3<f32>(0.0080, 0.0092, 0.0135)
  let amb   = u.ambientColor.rgb  * 0.032;  // vec3<f32>(0.030, 0.028, 0.032)
  ...
  if (r < 1.004 + mfEdgeD(u.edgeSoftness)) {
    ...
    // Quantise the field into plateaus. `lev` names the terrace, `stp` is where
    // inside it this pixel falls — the riser is at both ends.
    let d   = tmRotY(N, t * 0.032);
    let f   = tmTerf(d);
    let lev = floor(f * u.terraces);
    let stp = fract(f * u.terraces);

    // One hashed normal per terrace: what makes the steps read as facets of a
    // hopper crystal rather than contour lines painted on a sphere.
    ...
    // The heat pulse: a gaussian ring in angle from one hot pole, its front
    // travelling outward each cycle and cooling as it goes.
    let ph    = fract(t * 0.042);
    let front = sqrt(ph) * 3.0;
    let hotd  = normalize(vec3<f32>(-0.34, -1.0, 0.22));
    let ang   = acos(clamp(dot(d, hotd), -1.0, 1.0));
    // Written as q*q rather than pow(q, 2) — q is negative ahead of the front,
    // and pow with a negative base is undefined.
    let q  = (ang - front) / max(u.heat, 0.02);
    var Tt = exp(-q * q) * (1.0 - ph * 0.30);
    Tt = Tt + exp(-ang * 1.6) * (1.0 - ph) * 0.45;

    // Oxide thickness in nanometres, then thin-film interference at three
    // representative wavelengths — the temper colour sequence itself. Red,
    // green and blue band at different rates as the oxide thickens, and the
    // sweep between them is straw, bronze, violet, blue.
    //
    // The triple is per-channel WAVELENGTH in nanometres, and it divides, so
    // `spectrum` interpolates it toward its first component rather than toward
    // one: at 1 this is exactly (612, 548, 462) and the render is untouched, at
    // 0 all three channels read at 612nm and the interference goes greyscale
    // with its banding pattern intact (`filmColor` says what colour it becomes),
    // above 1 the channels separate further.
    ...
    // `filmColor` tints the film everywhere it is read — the metal mix below,
    // the terrace edges and the grazing term. White is a no-op.
    ...
    // The palette IS the temper colour — the oxide film on the steel, which is
    // this orb's entire subject. Written in WAVELENGTHS rather than frequencies:
    // cos(4pi * 2.55 * ox / wl), so the red channel completes one cycle every
    // ox = 120 and that is the ramp's parameter. Read cyclically, because an
    // oxide film bands round and round as it thickens. A real look change, like
    // the other interference orbs. SELECTED, never branched.
