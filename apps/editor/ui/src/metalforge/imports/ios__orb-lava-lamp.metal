...
constant float LQO_LOOP  = 9.0;
constant float LQO_SOFT  = 1.8;
constant float LQO_GRAIN = 0.05;
...
static float lqo_balls(float2 q, float ANG, float seed,
    ...
    for (int i = 0; i < 12; i++) {
        ...
        float  dir = select(-1.0, 1.0, h2 < 0.5);
    ...
    return saturate(smoothstep(0.5, 1.4, fs)
                    + 0.08 * (lqo_fbm(q * 3.0 + float2(seed, seed)) - 0.5));
}
...
[[ stitchable ]] half4 orbLavaLamp(float2 position,
                                   half4  color,
                                   float4 boundingRect,
                                   float  time,
                                   float  speed,
