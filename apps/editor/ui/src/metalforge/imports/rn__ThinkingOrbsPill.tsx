...
import { Platform, Text, View, useColorScheme, type ViewStyle } from "react-native";
...
function orbSheet(): any {
    "worklet";
    const g = globalThis as any;
    if (g.__mfThinkingOrbs) return g.__mfThinkingOrbs;
...
function orbDots(style: any, t01: any, S: any, dotScale: any, Q: any) {
  ...
  return out
    ...
    g.__mfThinkingOrbs = { ORB_STYLES, sizeDotScale, orbDots };
    return g.__mfThinkingOrbs;
}

const SHEET = orbSheet();
const STYLES = SHEET.ORB_STYLES;

function orbInk(hex: string): number[] {
    const h = hex.replace("#", "");
...
export const THINKING_ORBS_DEFAULTS = {
    style: LOOP_ID,
    ...
    dotScale: DOT_SCALE,
    knobs: KNOBS,
    scheme: SCHEME as "auto" | "light" | "dark",
    accent: ACCENT_COLOR,
    inkDark: INK_DARK,
    inkLight: INK_LIGHT,
};

export const THINKING_ORBS_MONO = MONO;
...
function orbFrame(
    ...
    "worklet";
    const sheet = orbSheet();
    const list = sheet.orbDots(index, t01, ball, sheet.sizeDotScale(ball) * dotScale, knobs);
    const inks = [
        new Float32Array([dot[0], dot[1], dot[2], 1]),
        new Float32Array([acc[0], acc[1], acc[2], 1]),
    ];
    ...
    for (let i = 0; i < list.length; i++) {
        const d = list[i];
        paint.setColor(inks[d.accent ? 1 : 0]);
        paint.setAlphaf(d.a);
...
function OrbBallLive({
    index, period, speed, reverse, startAt, ball, dotScale, knobs, dot, acc,
}: OrbBallProps) {
    ...
    const picture = useDerivedValue(() => {
        ...
        return orbFrame(
            index,
            orbPhase(period, speed, reverse, startAt, clock.value / 1000),
            ball,
            dotScale,
            knobs,
            dot,
    ...
    }, [clock, index, period, speed, reverse, startAt, ball, dotScale, knobs, dot, acc]);
    return (
        <Canvas style={{ width: ball, height: ball }}>
            <Picture picture={picture} />
        </Canvas>
    );
}

function OrbBallStill({
    index, period, speed, reverse, startAt, ball, dotScale, knobs, dot, acc,
}: OrbBallProps) {
    const picture = useMemo(
        () =>
            orbFrame(
                index,
                orbPhase(period, speed, reverse, startAt, 0),
                ball,
                dotScale,
                knobs,
                dot,
...
export default function ThinkingOrbsPill({
    ...
    const isDark = scheme === "auto" ? systemScheme !== "light" : scheme === "dark";
    ...
    const knobs = useMemo(
        ...
        [dots, spread, perspective, depthSize, depthFade, turn, tilt, spin, dotOpacity],
    );

    const scale = size / BALL;
    const gap = GAP * scale;
    const padTop = PAD_TOP * scale;
    const padBottom = PAD_BOTTOM * scale;
    const padSide = showsLabel ? PAD_LEFT * scale : padTop;
    const padEnd = showsLabel ? PAD_RIGHT * scale : padTop;
    const ballProps = {
        index,
        period,
        speed,
        ...
        >
            <View
                style={{
                    ...
                    alignItems: "center",
                    gap,
                    paddingTop: showsPill ? padTop : 0,
                    paddingBottom: showsPill ? padBottom : 0,
                    paddingLeft: showsPill ? padSide : 0,
                    paddingRight: showsPill ? padEnd : 0,
                    borderRadius: 999,
                    backgroundColor: showsPill ? pill : "transparent",
                }}
            >
                {animated ? <OrbBallLive {...ballProps} /> : <OrbBallStill {...ballProps} />}
                {showsPill && showsLabel ? (
                    <Text
                        style={{
                            fontFamily: MONO,
                            fontSize: FONT_SIZE * scale,
                            lineHeight: FONT_SIZE * scale,
                            color: labelInk,
