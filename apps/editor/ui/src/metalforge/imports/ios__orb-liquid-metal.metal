...
static float lm_sstep(float e0, float e1, float x) {
    float t = clamp((x - e0) / (e1 - e0), 0.0, 1.0);
    return t * t * (3.0 - 2.0 * t);
}

static float lm_hash(float3 p) {
    return fract(sin(dot(p, float3(127.1, 311.7, 74.7))) * 43758.5453);
}

static float lm_noise(float3 p) {
    float3 i = floor(p);
    float3 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = lm_hash(i);
...
static float3 lm_aces(float3 x) {
    return saturate((x * (2.51 * x + 0.03)) / (x * (2.43 * x + 0.59) + 0.14));
}

static float lm_schlick(float ct, float f0) {
    return f0 + (1.0 - f0) * pow(max(1.0 - ct, 0.0), 5.0);
}

static float lm_smin(float a, float b, float k) {
    float h = clamp(0.5 + 0.5 * (b - a) / k, 0.0, 1.0);
    return mix(b, a, h) - k * h * (1.0 - h);
}

static float lm_sphExit(float3 p, float3 d) {
    float b = dot(p, d);
    return -b + sqrt(max(1.0 - dot(p, p) + b * b, 0.0));
}

static float3 lm_knead(float t, float a, float b, float c, float ph) {
    return float3(sin(t * a) + 0.55 * sin(t * a * 0.41 + ph),
                  cos(t * b) + 0.55 * cos(t * b * 0.37 + ph * 1.7),
                  sin(t * c + ph * 2.3));
}

static float3 lm_studioBG(float2 p, float3 wall0, float3 wall1,
...
static float3 lm_bgThrough(float2 uv, float3 N, float3 wall0, float3 wall1,
                           float3 lamp1, float3 lamp2) {
...
static float3 lm_glassHi(float3 N, float2 su, float z, float3 spec,
...
static float lm_mb(float3 p, float t, float k, float drops, float ripple) {
    float d = length(p - float3(sin(t * 0.31) * 0.15, cos(t * 0.26) * 0.13, sin(t * 0.22) * 0.13)) - 0.29 * drops;
    d = lm_smin(d, length(p - float3(cos(t * 0.24 + 1.1) * 0.22, sin(t * 0.33 + 0.4) * 0.19, cos(t * 0.29) * 0.15)) - 0.21 * drops, k);
    d = lm_smin(d, length(p - float3(sin(t * 0.19 + 2.4) * 0.25, cos(t * 0.21 + 2.0) * 0.23, sin(t * 0.27 + 1.0) * 0.17)) - 0.16 * drops, k * 0.9);
    return d + (lm_fbm(p * 3.4 + float3(0.0, t * 0.11, 0.0)) - 0.5) * ripple;
}

static float3 lm_mbN(float3 p, float t, float k, float drops, float ripple) {
    float2 e = float2(0.0035, 0.0);
    return normalize(float3(lm_mb(p + e.xyy, t, k, drops, ripple) - lm_mb(p - e.xyy, t, k, drops, ripple),
                            lm_mb(p + e.yxy, t, k, drops, ripple) - lm_mb(p - e.yxy, t, k, drops, ripple),
                            lm_mb(p + e.yyx, t, k, drops, ripple) - lm_mb(p - e.yyx, t, k, drops, ripple)));
}

[[ stitchable ]] half4 orbLiquidMetal(float2 position,
                                      half4  color,
                                      float4 boundingRect,
                                      float  time,
                                      float  speed,
    ...
    if (r < 1.004 + mf_edge_d(edgeSoftness)) {
        ...
        if (hit > 0.5) {
            ...
            inner = env * select(mix(float3(metalColor.rgb), float3(fresnelColor.rgb), fr),
