...
const float blobs = 6.0;
const float blobSize = 1.0;
const float blobVary = 1.0;
const float wobble = 0.0;
const float shimmer = 0.12;
const float refraction = 0.65;
const float contrast = 1.0;
const float bias = 0.0;
const float fringe = 0.4;
const float iridescence = 0.0;
const float rim = 0.75;
const float glint = 0.6;
const float innerGlow = 0.6;
const float halo = 0.25;
const float grain = 1.0;
const float seed = 18.0;
const float exposure = 1.0;
const float edgeSoftness = 0.005;
const float edgeGlow = 0.0;
const float paletteCount = 0.0;
const float2 light = float2(-0.5, -0.86);
const half4 colorA = half4(0.180392, 0.062745, 0.396078, 1.0);
const half4 colorB = half4(0.984314, 0.443137, 0.521569, 1.0);
const half4 colorC = half4(0.992157, 0.729412, 0.454902, 1.0);
...
const half4 glowColor = half4(0.984314, 0.443137, 0.521569, 1.0);
const half4 paletteStop0 = half4(0.180392, 0.062745, 0.396078, 1.0);
const half4 paletteStop1 = half4(0.321569, 0.152941, 0.447059, 1.0);
const half4 paletteStop2 = half4(0.658824, 0.294118, 0.498039, 1.0);
const half4 paletteStop3 = half4(0.941176, 0.419608, 0.517647, 1.0);
const half4 paletteStop4 = half4(0.992157, 0.454902, 0.513725, 1.0);
const half4 paletteStop5 = half4(1.0, 0.541176, 0.47451, 1.0);
const half4 paletteStop6 = half4(1.0, 0.666667, 0.447059, 1.0);
const half4 paletteStop7 = half4(0.992157, 0.729412, 0.454902, 1.0);
const half4 paletteStop8 = half4(0.992157, 0.729412, 0.454902, 1.0);
const half4 paletteStop9 = half4(0.992157, 0.729412, 0.454902, 1.0);
const half4 paletteStop10 = half4(0.992157, 0.729412, 0.454902, 1.0);
const half4 paletteStop11 = half4(0.992157, 0.729412, 0.454902, 1.0);
...
const float LQO_LOOP  = 9.0;
const float LQO_SOFT  = 1.8;
const float LQO_GRAIN = 0.05;
...
float lqo_balls(float2 q, float ANG, float seed,
                float count, float size, float vary) {
    float fs = 0.0;
    for (int i = 0; i < 12; i++) {
        float  fi  = float(i);
        if (fi >= count) { break; }
        float  h1  = lqo_hash(float2(fi * 1.3, seed + 2.0));
        float  h2  = lqo_hash(float2(seed + 7.0, fi * 2.1));
        float  k   = floor(h1 * 3.0) + 1.0;
        float  dir = (h2 < 0.5) ? 1.0 : -1.0;
        float2 c0  = 0.34 * float2(cos(LQO_PI2 * h1), sin(LQO_PI2 * h2));
        float  orb = 0.16 + 0.22 * h2;
        float2 c   = c0 + orb * float2(cos(dir * k * ANG + LQO_PI2 * h1 * 3.0),
                                       sin(dir * k * ANG + LQO_PI2 * h1 * 3.0));
        float  br  = (0.14 + 0.12 * h1 * vary) * size;
        fs += br * br / (dot(q - c, q - c) + 0.003);
    }
    return clamp(smoothstep(0.5, 1.4, fs)
                 + 0.08 * (lqo_fbm(q * 3.0 + float2(seed, seed)) - 0.5), 0.0, 1.0);
}
...
half4 main(float2 position) {
    ...
    float n1 = lqo_balls(q0, ANG, seed, blobs, blobSize, blobVary);
...
export default function OrbLavaLampView() {
