...
[[ stitchable ]] half4 orbPyre(float2 position,
                               half4  color,
                               float4 boundingRect,
                               float  time,
                               float  speed,
    ...
    c = select(c, mf_ramp_linR(inten / 1.30, pal), paletteCount > 0.5);
    ...
    float3 col = saturate(c) * ballA;
    col = saturate(col * max(exposure, 0.0));
