...
constant float LQO_LOOP  = 7.0;
constant float LQO_SOFT  = 1.4;
constant float LQO_GRAIN = 0.055;
...
static float3 lqo_pal(float x, float3 A, float3 B) {
    float s = saturate(x);
...
[[ stitchable ]] half4 orbMagenta(float2 position,
                                  half4  color,
                                  float4 boundingRect,
                                  float  time,
                                  float  speed,
