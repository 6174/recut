...
const float waveFreq = 1.0;
const float amplitude = 1.0;
const half4 tint = half4(0.176471, 0.031373, 0.701961, 1.0);
const half4 depth = half4(0.509804, 0.243137, 0.921569, 1.0);
const half4 highlight = half4(1.0, 1.0, 1.0, 1.0);

float2 sphIntersect(float3 ro, float3 rd, float rad) {
    float b = dot(ro, rd);
...
float waveDisp(float3 p, float t, float fr, float amp) {
...
float mapScene(float3 p, float t, float fr, float amp) {
    return length(p) - 1.0 - waveDisp(p, t, fr, amp);
}

float3 calcNormal(float3 p, float t, float e, float fr, float amp) {
...
half4 main(float2 position) {
    ...
    if (hh.x < 0.0) return half4(0.0, 0.0, 0.0, 1.0);
    ...
    if (!hit) return half4(0.0, 0.0, 0.0, 1.0);
