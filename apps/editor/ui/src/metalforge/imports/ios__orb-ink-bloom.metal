...
static float ib_sstep(float e0, float e1, float x) {
    float t = clamp((x - e0) / (e1 - e0), 0.0, 1.0);
    return t * t * (3.0 - 2.0 * t);
}

static float ib_hash(float3 p) {
    return fract(sin(dot(p, float3(127.1, 311.7, 74.7))) * 43758.5453);
}

static float ib_noise(float3 p) {
    float3 i = floor(p);
    float3 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    float a = ib_hash(i);
...
[[ stitchable ]] half4 orbInkBloom(float2 position,
                                   half4  color,
                                   float4 boundingRect,
                                   float  time,
                                   float  speed,
    ...
    if (r < rad + 0.01 + mf_edge_d(edgeSoftness)) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            float3 fr  = select(0.5 - 0.5 * cos(6.2831 * ith * freq),
                                mf_ramp_cycR(ith, pal), paletteCount > 0.5) * float3(filmColor.rgb);
