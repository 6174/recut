/**
 * [INPUT]: 依赖 class-variance-authority、Button 和 UI 样式工具。
 * [OUTPUT]: 对外提供带统一尺寸、焦点状态与可选辅助操作的 Input。
 * [POS]: components/ui 的基础文本输入控件，所有直接文本和 HEX 编辑入口以它对齐。
...
const inputVariants = cva(
	"file:text-foreground placeholder:text-muted-foreground border-border bg-background flex w-full min-w-0 rounded-xs border shadow-xs outline-none file:inline-flex file:border-0 file:bg-transparent file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 focus-visible:ring-offset-0 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
	{
		variants: {
			variant: {
				default: "selection:bg-primary selection:text-primary-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30",
			...
			>
				...
				{showClear && (
					<Button
						...
						aria-label={t(locale, "ui.clearInput")}
					>
				...
				{showPasswordToggle && (
					<Button
						...
						aria-label={showPassword ? t(locale, "ui.hidePassword") : t(locale, "ui.showPassword")}
					>
						{showPassword ? (
