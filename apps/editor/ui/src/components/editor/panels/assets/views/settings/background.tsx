...
const BlurPreview = memo(
	({
		...
		blur: { label: string; labelKey: import("@/i18n").I18nKey; value: number };
		isSelected: boolean;
		onSelect: () => void;
	}) => {
		const canvasRef = useRef<HTMLCanvasElement>(null);
		const locale = useRecutLocale();
		const label = t(locale, blur.labelKey);
		...
		return (
			<button
				...
				onClick={onSelect}
				type="button"
				aria-label={t(locale, "settings.bg.selectBlur", { label })}
			>
				<canvas
					ref={canvasRef}
			...
			() =>
				backgrounds.map((bg) => (
					<button
						...
						onClick={() => onSelect(bg)}
						type="button"
						aria-label={t(locale, "settings.bg.selectBackground", { name: bg })}
					/>
				)),
			[
				backgrounds,
...
const SECTION_TITLE_KEYS: Record<string, import("@/i18n").I18nKey> = {
	"colors": "settings.bg.colors",
	"pattern-craft": "settings.bg.patternCraft",
	"syntax-ui": "settings.bg.syntaxUi",
};
...
export function BackgroundContent() {
	...
	return (
		<div className="flex flex-col">
			...
			>
				<SectionHeader>
					<SectionTitle>{t(locale, "settings.bg.blur")}</SectionTitle>
				</SectionHeader>
				<SectionContent>
			...
			{COLOR_SECTIONS.map((section) => (
				...
				>
					<SectionHeader>
						<SectionTitle>{t(locale, SECTION_TITLE_KEYS[section.id])}</SectionTitle>
					</SectionHeader>
					<SectionContent>
