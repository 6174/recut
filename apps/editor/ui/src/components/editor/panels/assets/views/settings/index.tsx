/**
 * [INPUT]: 依赖项目设置、画布尺寸草稿与统一 NumberField。
 * [OUTPUT]: 对外提供 SettingsView，编辑项目帧率、画布与背景设置。
 * [POS]: assets/settings 的项目配置视图，画布尺寸遵循全局数值输入与拖拽规范。
	...
	return (
		<PanelView
			...
			actions={
				...
				>
					<TabsList>
						<TabsTrigger value="project-info">{t(locale, "settings.projectInfo")}</TabsTrigger>
						<TabsTrigger value="background">{t(locale, "settings.background")}</TabsTrigger>
					</TabsList>
				</Tabs>
			}
		>
			{view === "project-info" && (
				<div className="flex flex-col">
					<Section showTopBorder={false}>
						<SectionHeader>
							<SectionTitle className="flex-1">{t(locale, "common.name")}</SectionTitle>
