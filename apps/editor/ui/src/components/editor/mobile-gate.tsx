	...
	return (
		<div className="bg-background relative flex h-screen w-screen flex-col overflow-hidden">
			...
			>
				...
				<span className=" text-sm">{t(locale, "mobile.goBack")}</span>
			</Button>

			<div className="flex flex-1 flex-col justify-center gap-5 px-7">
				<div className="flex flex-col gap-3">
					<h1 className="text-foreground text-3xl font-bold tracking-tight">
						{t(locale, "mobile.desktopOnly")}
					</h1>
					<p className="text-muted-foreground text-sm leading-relaxed">
						{t(locale, "mobile.desc")}
					</p>
				</div>
				<div className="flex items-center gap-3">
					<Button onClick={handleContinue}>{t(locale, "mobile.takeLook")}</Button>
					<Button variant="ghost" asChild>
						<Link href="/roadmap" className="flex items-center gap-1">
							{t(locale, "mobile.roadmap")}
