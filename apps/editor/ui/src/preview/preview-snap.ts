...
export function snapPosition({
	...
	/**
	 * 本次移动前的当前位置：提供时"只有比当前位置更接近目标才吸附"（吸附只拉、不拉住离开）。
	 * 缺省（mask 等场景）保持旧语义：阈值内即吸附。
	 */
	...
	function getClosestAxisSnap({
		...
		const snapCandidatesWithinThreshold = candidates.filter((candidate) => {
			if (candidate.distance > threshold) return false;
			if (current === undefined) return true;
			// 吸附只会把元素拉向目标；若本次移动在"远离目标"（不比当前更近），不吸附，
			// 避免元素停在画布中心时被吸附"按住"，一旦越过阈值又突跳。
			const currentDistanceToTarget = Math.abs(current - candidate.snappedPosition);
			return candidate.distance < currentDistanceToTarget;
		});
