/**
 * [INPUT]: 依赖选中任务详情（modal.task.get）、生成产物（modal.generation.complete）、运行参数（modal.task.params）、持久日志（modal.task.logs）与 recut.media.preview 全屏预览
 * [OUTPUT]: Right 面板：任务头（状态/取消）+ 生成预览（按 output.kind 渲染图片/视频/音频；图片可全屏预览并含「以此为参考图运行」「重新调整参数」入口）与运行参数回显 + 入库 / 部署下载实时日志
