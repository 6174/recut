...
gen-studio/: 仓库内目录（非 submodule）；内置的本地生成 App（`recut.gen-studio`「生成工坊」），一个 runtime（专属 venv，默认 diffusers）托管多个开源模型，先准备环境再按需下载权重（Hugging Face / ModelScope / 自动）。第一版兑现 Qwen-Image 文生图；经 manifest `contributes.media` 声明 `local-gen` provider，平台据此把模型并入全局生图目录。模型位于 `~/.recut/models/gen-studio/`。
