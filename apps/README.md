...
成员清单
ai-short-film/: `git@github.com:6174/recut-ai-short-film.git` 的 Git submodule（跟踪 main）；AI 短片项目型应用，冻结导演风格模板后依次完成全局资料研究、用户确认、创作方案选定、剧本与场景方案、媒体生成与可供 Remotion 消费的短片交接包，主仓库只固定 commit。
...
comfyui-studio/: 仓库内目录（非 submodule）；内置的本地 ComfyUI 工作流 App（`recut.comfyui-studio`「ComfyUI 工作台」），一个共享 runtime（专属 venv + 固定 commit 的 ComfyUI 源码）托管多个自包含 comfyui workflow app（`comfyuiapps/<id>/`：manifest 表单/权重/输出 + workflow.py 动态构图 + bootstrap.py 依赖）；先准备环境（并起 ComfyUI 引擎）再按需下载权重（Hugging Face / ModelScope / 自动）。第一版兑现 Qwen-Image-2.1 文生图/编辑；经 manifest `contributes.media` 声明 `local-gen` provider，平台据此把模型并入全局生图目录。模型位于 `~/.recut/models/comfyui-studio/`。
