...
Recut 是一个**本地优先、开源、可扩展的 AI 视频创作工作台**。给 Recut 一个想法、一条参考视频或者一个故事，AI 自动完成调研、策划、生成、剪辑与成片。
...
一条参考视频、一个想法，或者一个世界观——无论手里有什么，Recut 都能把它变成一条成片。
...
把角色、故事与风格做成一个 World，固化成一个可长期复用的永久资产，而不是每次从零生成。此后不断迭代，持续产出各平台、各语言的新视频。
...
1. **给出一个起点**：丢进一条参考视频、写下想法，或选一个世界观；也可以直接在 App 中选择素材、模板与参数。
