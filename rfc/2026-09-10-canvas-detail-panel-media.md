...
- LibLib（图 3）：画布内图下方浮层提供 prompt 输入框 + "本地上传 / 选择素材历史"。
...
### 3.2 核心抽象：历史 = "当前 asset 指针" 的变更历史（History ⊆ Asset）

两个关键对齐决策：

1. **底层统一为 Asset**：无论是 AI 生成产出、还是用户本地上传，落库都是一枚 `/v1/media/assets` 资产；一次生成 = 一枚 asset（同一 job 的多张产出 = 多枚 asset）。
2. **历史是"当前素材是什么"的历史，不是"生成历史"**：图片的内容可能来自生成、也可能来自用户上传/素材库，因此历史记录的粒度是**指针切换事件**——任何来源的 asset 被写回元素的那一刻， 都记一次变更。"生成出来的 asset"只是其中一类入列者，与上传/素材库选取完全同构：历史 = 元素曾指向过的 asset 集合（最近优先），资产本体永远以素材库为唯一真相。

- 作为"生成产物"的 asset 携带配方（recipe）：`metadata.recipe`（provider、model、prompt、参数、参考素材 ids、上下文备注）；上传素材 recipe 为空（prompt 空、参考即自身）。
- **当前引用与历史的分离**：画布元素/attr/字段只存"当前 assetId"（单一指针）；历史 = 按 elementId 记录的曾用 assetId 集合（本地持久化指针栈），"换图只是改指针 + 记一条"，旧 asset 不消灭，跨设备可在素材库找回同一份数据。
- "素材历史"视图与素材库选择器看到的是同一份资产数据，避免双份真相。

由此产生的行为含义：
- 历史"设为当前" = 把该 assetId 写回元素（指针切换 + 再记一条，O(1)、可撤销）；
- 历史"删除" = 删除这一枚 asset（素材库同源操作，被引用时给保护提示，素材库现有删除逻辑对齐）；
- 来源无关：AI 生成、上传、素材库选择进历史的方式完全一致。
...
- 入口二处：Panel 内历史列表本身；预览区上的 hover 菜单"选择素材历史"（对标 LibLib 的换图入口）。
