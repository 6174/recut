...
- 实施进展：**P0 已落地**（根布局 `title.template` + 默认 noindex、四个 Marketing 路由独立 metadata、canonical、`app/robots.ts`/`app/sitemap.ts`/`app/manifest.ts`、Worker www→裸域 301、`/marketing` 301、Marketing/App Host 未知路径 404、`not_found_handling = "404-page"`）。**P1 已落地**（全站 og/twitter 标签 image 指向 `logo.jpg`，`marketing-jsonld.tsx` Organization/WebSite/SoftwareApplication/Blog/BlogPosting/Breadcrumb，`icon.png`/`apple-touch-icon.png`/`favicon.ico` + `theme-color`）。**P2 已落地**（Blog 18 篇 + App 详情页全部 MDX 化，`BlogPostContent` 与 App 详情共用 `MarkdownContent` 渲染器 + `ShareActions` 分享组件）。**扩展落地**：官网新增公开 `/apps` 应用市场与 `/apps/:appID` SEO 落地页（Worker 从 Marketing Host 映射、robots 不再 Disallow `/apps`、sitemap/JSON-LD 覆盖、header/footer 加入口）；App 内容与工作台 `app-catalog` 解耦并 MDX 化（`content/apps/zh-CN/`，frontmatter 承载元数据/FAQ/要求，正文自由 markdown）；Blog 内容按关键词策略扩充至 18 篇，其中 8 篇「技术关键词碰瓷」系列由深度研究产出（CosyVoice / Qwen ASR / Depth Anything / AI 封面 / Whisper / FFmpeg / Remotion / 剪映平替 / 本地隐私 / MiniMax-ElevenLabs / Remotion 教程 / 字幕工具对比 / 开源工具盘点，覆盖自部署难的高搜索词并落点到免配置 App）；内容全部迁移为 MDX（`content/marketing/<locale>/`），`lib/marketing-posts.ts` 与 `lib/marketing-apps.ts` 改为 server-only 加载器（fs + gray-matter），客户端组件经 props 注入数据，`content-locale.ts` 预留多语言；首页按「本地 AI 视频剪辑」关键词策略重构（title/description/H1/H2、三步开始、适合谁、与云端对比、FAQ 六条），并新增首页 FAQPage、应用 ItemList 与增强 SoftwareApplication JSON-LD；BlogPosting/Article 补 `publisher.logo` 支持文章富结果；全站接入 PostHog 埋点（autocapture general 点击 + capture_exceptions 报错 + `recut_page_viewed` 页面访问覆盖官网/工作台 + `trackEvent` 语义事件）。验证：`make build:cloudflare` 全绿（42 页），逐页 head/sitemap/robots 审计通过、MDX 代码块渲染与无 client 引入 fs 均通过，Worker 路由单测（含 deep-link 不回归）。
