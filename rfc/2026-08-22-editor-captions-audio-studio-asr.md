...
- 状态：**部分实施（2026-08-22）**——P0 已按本稿落地：service `ctx.capabilities.invoke/inspect`（通用能力桥，复用 AppHost.invoke，含统一错误信封/同步超时/审计）＋能力级测试；audio-studio `audio.transcribe(saveToLibrary)`+capability 标记+懒终态入库/幂等去重；editor `subtitle.generate/status/commit/capabilities` 一组 op + 字幕轨 `captionSource` 引用 + UI 生成立即链路 + 未安装统一安装引导（宿主全局 `AppInstallGuide` 弹窗 + iframe `apps.request-install`）；`web` 侧 tsc 零错、service `go test` 全绿、editor `vite build` 通过。P1（授权签名、部分成功 repair）、P2（整轨混音、`local_speech_bridge` 收敛）未实施。
- 作者：Recut
- 日期：2026-08-22
