...
- 状态：**部分实施（2026-08-22）**——P0 已按本稿落地：service `ctx.capabilities.invoke/inspect`（通用能力桥，复用 AppHost.invoke，含统一错误信封/同步超时/panic 兜底/审计）＋能力级测试；audio-studio `audio.transcribe(saveToLibrary)`+capability 标记+懒终态入库/幂等去重；editor `subtitle.generate/status/commit/capabilities` 一组 op + 字幕轨 `captionSource` 引用 + UI 生成立即链路 + 未安装统一安装引导（宿主全局 `AppInstallGuide` 弹窗 + iframe `apps.request-install`）；`web` 侧 tsc 零错、service `go test` 全绿、editor `vite build` 通过。**体验修订（同日）**：字幕面板暗色可读性（caution 块改高对比前景色 + 宿主弹窗显式深色遮罩）；模型选择只列 audio-studio 已安装模型、空时 CTA 去声音工坊安装；新增「整个时间线（混音）」目标（浏览器提取混音 → 上传全局 audio 素材 → 转写，CapCut 式全轨字幕）；**能力实时重检**（切回字幕 Tab / 点生成 / 手动「重新检测」都刷新快照；reason 区分 not-installed / stale（版本过旧）/ env / no-model / ready，避免安装后旧快照误导）。P1（授权签名、部分成功 repair）、P2 的 `local_speech_bridge` 收敛未实施。
- 作者：Recut
- 日期：2026-08-22
