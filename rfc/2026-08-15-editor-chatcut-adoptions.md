...
- 实施进展：**P1 已落地**（技能层：speech-editing/subject-protection/errors references + SKILL.md/directing/preview-export 纪律）。**P2 核心已落地**：`ctx.media.transcript`（service runtime）、speech-track（元素 `transcript` 字段 + `transcript-attach`/`transcript-fix` op）、`script.read/apply/clean/find/fix-transcript/attach` 六个 mcp 操作、`timeline.validate` 增 `transcript-src`、manifest 注册、L0 全绿（`node apps/editor/scripts/test-model-api.js`）+ `parseTranscriptSrt` Go 单测。P2 未覆盖：多元素说话 run 的手工重排（当前按序重建）、词级停顿压缩（需词级时间）。**P3 已落地**：`track-role` op（undoable）+ 轨道 `role`/`audioRouting` 数据模型、`buildDuckEnvelope` 纯函数（background.js 与 UI `timeline/duck.ts` 同构，Preview==Export）、`audio.smooth`（音量关键帧微淡入淡出，幂等）、UI AudioManager 预览与 `createTimelineAudioBuffer` 导出双路径接入 duck、L0 + `TestEditorAudioMix` 集成测试全绿。P3 未覆盖：duckDepthDb 自动初始化只按 anchor 平均响度粗估（未做真实响度测量）。**P4（library.browse 内置目录）待做**。
