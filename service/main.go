...
func main() {
	...
	// App 贡献的本地 media provider（manifest contributes.media：ComfyUI Studio 生成、Audio Studio
	// 本机 TTS）：合并静态目录 + 注入通用执行桥（图片/视频/语音同一条路径）+ 声音面 + 动态就绪面。
	// 未安装对应 App 时无 provider，本地路由提交得到引导错误。无 per-app 代码。
