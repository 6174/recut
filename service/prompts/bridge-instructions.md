...
- 用 `recut.skills.list` / `recut.skills.read` 加载匹配的工作流，再调用对应 `appId.operation` 工具。skill 不都属于某个 App：平台能力（World / 画布、导演类等）的技能以 `appId="recut.platform"` 并入 `recut.context.skills`（`recut.skills.list` 同）；`recut.context` 已返回时优先据此直接 `recut.skills.read`，不要因为「没有对应 App」就跳过。涉及 World / 画布（世界实体、属性、媒体生成落画布）时先读 `recut-worlds`。
