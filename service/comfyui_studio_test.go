/*
 * [INPUT]: 依赖 BuiltinAppManager/LoadCatalog、comfyui-studio 的 contributes.media 声明与 media 包的
 *          RegisterAppProviders/CapabilityModelGroups
 * [OUTPUT]: 验证 comfyui-studio 随客户端内置安装、其 contributes.media 声明的 local-gen provider 被映射为
...
func TestComfyUIStudioContributesLocalMediaProvider(t *testing.T) {
	...
	app, ok := catalog.Get("recut.comfyui-studio")
	if !ok {
		t.Fatal("comfyui-studio was not installed as a built-in App")
	}
	if app.Manifest.Contributes == nil || app.Manifest.Contributes.Media == nil {
		t.Fatal("comfyui-studio must declare contributes.media")
	}
