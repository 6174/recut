...
runtime.go: 在 Goja sandbox 中执行 App background.js，并注入统一 operation 注册器；同一 handler 可按 manifest surface 暴露给 UI API 与 MCP；获授权的 App 可将已完成 Asset materialize 到私有文件、在用户确认后以流式 reader import 私有输出，避免长视频占满内存，并使用通用 `ctx.shell` Job 或 manifest 声明的 `ctx.python` venv，模型根固定注入为 `~/.recut/models`。
...
bridge.go: 管理 Agent session 与本地 CLI 连接，在中立会话工作区物化全局 guide 与 Codex/Claude/OpenCode 的 MCP 配置，会话不绑定任何项目，项目与 App 全部由 MCP 上下文工具发现；OpenCode 的 MCP 初始化与工具调用统一配置为 5 分钟，并在会话 `opencode.json` 中将 `permission.external_directory` 设为 `allow`，避免无头 CLI 卡在无人应答的目录权限询问（用户确认式访问留待后续阶段收紧）；支持把 guide 与 MCP 配置写入显式目录，供同一 OpenCode 原生会话跨 Turn 复用其原始工作区；原生素材库只使用平台模板，不读取任意工作目录的 AGENTS.md。
bridge_prompt_test.go: 锁定渲染后的 Vox Agent guide 必须使用 Recut 视频生成 API、包含中文 Vox 提示词/导演语言，且禁止把场景生成委托给 HyperFrames 或本地渲染；并锁定 OpenCode 会话工作区的 `permission.external_directory` 允许、原生会话续跑复用其原始工作区，以及工作区只在原生会话出现后才被固定。
prompts/: Go 后端私有的嵌入式平台 Agent 模板；不会作为 App 包内容或运行时外部依赖暴露。
agent.go: 保存本机用户的一对一 Agent 会话、消息、项目媒体引用、Codex 模型/推理强度与事件；会话完全不绑定项目，项目与 App 由 Agent 通过 MCP 上下文工具显式发现；Codex、Claude 与 OpenCode 的可执行位置共享 Store 的持久化定位缓存，缓存失效或启动失败才刷新；OpenCode 配置和每次执行仍读取本机 `opencode models`，只接受 TUI 当前真实可选的全部 provider/model，默认 Go DeepSeek V4 Flash，新会话显式标题以避免额外标题模型请求，并以 `--auto` 自动批准未被 OpenCode 明确拒绝的工具调用，使其与 Codex 的无人值守执行模式一致，并将模型连接重试映射为 UI 状态；每个 OpenCode 原生会话首次运行会固定其创建时的工作区，后续 Turn 一律 `--dir` 该原始工作区并用 `--session` 续跑——以不同目录续跑会令 `opencode run --session` 不输出事件并永久卡死，原生会话被清空时工作区一并清除；OpenCode 不设总执行时长上限，仅在连续 6 分钟没有 stdout/stderr 活动时取消并清除原生会话，避免坏的 provider stream 被下一条消息复用；会话详情先完整读取并关闭 Turn 结果集，再查询附件，因此单连接池也不会自锁；同一会话把生成期间的新消息持久化为 FIFO 待发送队列，消息、附件和会话运行态以单一短事务原子提交，停止操作原子取消 active 与 queued Turn、结束关联 Task、将会话收敛到 idle 并清除 OpenCode 原生会话后才终止运行时；服务日志审计会话和 Turn 的排队、开始、完成、取消及失败，但绝不写入用户消息或 CLI 输出；服务重启时取消无法跨进程恢复的 active Turn、把会话收敛为空闲并恢复安全的 queued Turn。Turn 携带泛化的消息上下文（MessageContext）：`type` 区分 media / page / 未来类型，`source` 区分用户选择（user）与自动附带的当前页面（page/app）；media 上下文以 assetId、类型、来源和只读路径同时交给 Agent，Codex 仅对图片注入原生图片参数，page 上下文按结构化标题/路径/选中/内容片段渲染进提示词，扩展新类型只需注册一个 materializer。并将 JSONL 规范化为分离的工具输入、输出、OpenCode `state.error` 与成本信息，供 UI 时间线完整查看。
