...
shell_jobs.go: 可恢复的非交互本地进程任务；以单锁保护状态/日志的落盘与读取，持久化 queued/running/terminal 状态和顺序 stdout/stderr JSONL 日志，投递项目事件，并在 service 日志记录不含命令参数的排队/终态审计；取消句柄在排队时即建立，队列和运行阶段都可可靠取消，daemon 重启时将未完成任务收敛为 interrupted。
...
bridge.go: 管理 Agent session 与本地 CLI 连接，在中立会话工作区物化全局 guide 与 Codex/Claude/OpenCode 的 MCP 配置，会话不绑定任何项目，项目与 App 全部由 MCP 上下文工具发现；OpenCode 的 MCP 初始化与工具调用统一配置为 5 分钟，并在会话 `opencode.json` 中将 `permission.external_directory` 设为 `allow`，避免无头 CLI 卡在无人应答的目录权限询问（用户确认式访问留待后续阶段收紧）；支持把 guide 与 MCP 配置写入显式目录，使 Codex/Claude/OpenCode 三种 runtime 都复用其原生会话的原始工作区续跑；原生素材库只使用平台模板，不读取任意工作目录的 AGENTS.md。
