...
bridge.go: 管理 Agent session 与本地 CLI 连接，在中立会话工作区物化全局 guide 与 Codex/Claude/OpenCode 的 MCP 配置，会话不绑定任何项目，项目与 App 全部由 MCP 上下文工具发现；OpenCode 的 MCP 初始化与工具调用统一配置为 5 分钟；原生素材库只使用平台模板，不读取任意工作目录的 AGENTS.md。
