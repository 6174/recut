...
project.go: 创建平台项目并按 App scope 提供 SQLite、文件与 Artifact 存储；素材库仅可创建内部系统项目，不能作为用户项目模板。
...
agent_server.go: 提供 Agent Session 的创建、Codex 会话模型/推理强度更新、带项目媒体资产引用的待发送消息入队、停止、查询与 SSE 事件 API，以及全局与按项目解析的新对话引导 API；仅对当前进程真实运行的回复接受停止。
