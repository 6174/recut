...
model_providers/: 按 Provider ID 分派的图片生成策略；`atlas-cloud` 走 Atlas 原生预测协议（提交 generateImage → 轮询 prediction → 下载 outputs[0]），`openai` / `openai-compatible` 走 OpenAI 兼容 `/images/generations`（有引用时 multipart `/images/edits`）；策略只负责线协议与取回最终字节，不接触工作区 SQLite、密钥存储或 Asset 持久化。
