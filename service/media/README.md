...
service.go: Workspace 端口、MediaService 组合根、图片等同步生成 5 分钟长请求与短状态查询分离的 HTTP 超时配置及少量跨层诊断入口。
