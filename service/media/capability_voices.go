 ...
 * [OUTPUT]: 对外提供 CapabilityVoiceGroups：按 capability 聚合所有可用声音分组（本地 provider + 每个云端凭据），含默认路由标记、provider 可选语音模型、逐组错误；本地分组的声音由各 provider 注册的 voices 面提供（App 的 contributes.media.voices）
...
func (m *MediaService) CapabilityVoiceGroups(capability MediaCapability) ([]CapabilityVoiceGroup, error) {
	...
	for _, provider := range m.Providers() {
		...
		if provider.Protocol == "local" {
			voices := []MediaVoice{}
			if fn := m.LocalVoiceProvider(provider.ID); fn != nil {
				voices = fn()
			}
