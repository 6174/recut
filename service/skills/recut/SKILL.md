...
- **导演意图**（凡计划里有生成图片/视频就必填）：`route` + 每个 shot/场景的 `viewer job`（它为什么存在）+ 媒介判定（生成 / motion-graphic / 字幕排版 / 混合）+ 段·场景单位与连续性 + 风格锁定（STYLE LOCK）与参考锚点；先加载 `recut-director` 得出，不要空着；
...
- `recut.worlds.list` / `recut.worlds.get` / `recut.worlds.entities.list`：Canon 相对稳定；自己 upsert 后增量，且这些写入本就要求用户明确授权，天然带失效信号。
