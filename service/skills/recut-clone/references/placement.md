...
clone 的**组装、字幕/图形、预览、校验、导出**全部交给 `timeline-editor`（`recut.editor.*` op + 它的技能 references）。本页只说明「clone 怎么把 `clone.md` 的锚点变成落轨」。锚点契约见 `anchors.md`。
...
- `recut.editor.workflow_context`（读一次）→ `timeline.read` 看现状。
...
1. **目标 A-roll 先定稿**：`script.attach` 绑转写 → `script.read` 物化 `scripts/timeline.md` → 确认文稿。
2. **算窗口**：按 `anchors.md` 把 `clone.md → Plan` 里每条 `speech@` / `clock@` 映射为 `[tlStart, tlEnd]`。
3. **落轨**：

| clone 产物 | 落到 timeline-editor |
|---|---|
| 媒体（已 `completed`） | `timeline.command` insert（video/image element，`assetId`），窗口取锚点映射 |
| 图形/MG（`verified`） | `timeline.placeComponents`，窗口取锚点映射 |
| 配音/配乐/SFX | `timeline.placeAudio` |
| 字幕/文案 | `subtitle.import`（样式走 `caption-style`） |
| 封面 | `cover.*`（若用户要求） |

- **不做手填秒数的顺序铺**：窗口一律来自锚点映射；无口播用 `clock@` 显式给区间。
- 同一 `speech@` 可落多条（分屏/图文叠加），按轨道避碰。
- 落轨后把实际窗口回填 `clone.md → Plan.tlWindow`。

## 4. 校验与交付

- **自检**：`timeline.validate` 零违规；受影响 settled frame 用 `preview.frame` 抽检构图/可读性。
- **交付**：`export.start` → `recut.job.wait` 到 `completed` → **实际观看**后报告。`editor-not-open` / headless 不可用时只能报告草稿。
...
4. `timeline.validate` + `preview.frame` 抽检；新窗口回填 `clone.md`。
