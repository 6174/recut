...
当前正式常驻版：**5.4.2**。它在5.4.1空间连续性架构之前增加“剧本理解→调度→镜头句→复合运镜”的镜头设计层。
**5.4.1原始快照保存在 `versions/5.4.1/`，需要时可按哈希完整回退。**  
**升级前的5.4.2候选快照保存在 `versions/5.4.2-candidate/`，只作本次正式化的回退证据。**
...
13. 把分镜写入严格六大模块；普通任务默认同轮交付，只有用户明确要求设计稿时才等待其确认。数字10十段式只作为信息内核，不拆成十个外显章节。
...
六、分段脚本
<按非等分时间逐段写机位、构图、摄影机、动作因果、焦点/遮挡、声音台词、光影、环境反馈与终点状态>
