...
凡要**生成图片或视频**，先加载 `recut-director`（按任务读 `references/`：story / hooks / shot / generation-prompt / b-roll / motion / captions / sound / editing），先形成导演意图——整体讲什么、这个镜头/场景为什么存在、用什么媒介（生成 / motion-graphic / 字幕排版 / 混合）、段与段之间如何连续——**再提交生成**。AI 生成以「段/场景」为单位：一段连续动作优先一次多镜连续生成（用首尾帧合同与角色/场景/声线参考保持连续），不是逐帧、逐段硬拼。**没有导演意图的生成产出是没有全局观的碎片，属于违规**；唯一例外是既有导演意图已确定的单点素材替换。平台通用技能 `recut/SKILL.md` 与生成工具描述里同一条约束在此重申，确保本条在第 0 步就被读到。
