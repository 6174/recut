...
- `insert` 用未验证的 `mediaId`/`sourceUrl` 会被 `timeline.validate` 拦截：先 `recut.media.list_assets` 拿真实 assetId，再重试 insert。
...
- 图片/语音默认**先落位、不空等**：多个媒体生成可以同时在途，只需记录 jobId，不必逐个长 wait；仅在依赖产物内容时才观察终态。长 wait 一次只维护**一个活跃观察**，不并行发多个长阻塞。
