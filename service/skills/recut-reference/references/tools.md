...
| `probe` / `frames` / `contactSheet` | M0 | 探测、抽帧、接触表 |
| `boundaries` / `clip` | M1 | 切点、源片段 |
| `words` / `measure` | M2 | 词级（可选）、估时 |
| `reference.create` / `reference.attach` | M3 | 标记参考、写观察 |

底层依赖：平台全局 Python venv（`<dataDir>/python/platform/<ver>`）提供 ffmpeg/ffprobe/PySceneDetect/opencv/Pillow。**平台启动只前置 PATH、不安装**；缺环境时工具返回「需要准备」错误 —— 按指引让用户准备，**不要自行 pip/uv 安装**。
...
- **平台页（YouTube/TikTok/B站…）**：平台无下载能力；默认由宿主 Agent 自理（自行用 yt-dlp），**这不是平台保证**。AI 下载的文件需导入为素材后才能进入理解链路 —— 目前「本地视频/音频文件 → 资产」的 MCP 入口缺失，缺失时如实说明。
...
| 现象 | 原因 | 处理 |
|---|---|---|
| 缺 ffmpeg/ffprobe | 平台 venv 未准备（或未装 ffprobe） | 让用户按 install/prepare 指引准备；不要自行安装 |
