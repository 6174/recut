> 来源: recut/apps/remotion-studio/skills/remotion-studio/references/shot-recipes/（自有存量，去介质化，原文语言保留）
...
| blur 驱动 | 帧间位移差分 `|p(f)-p(f-1)|`×系数 | 手排模糊关键帧必然对不上速度；差分法帧确定（模式已沉为 /remotion-kit/components/helpers/motion.ts velocityAt） |
