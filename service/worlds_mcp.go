...
func worldsMCPToolDefinitions(locale Locale) []map[string]any {
	tools := []map[string]any{
		...
		{"name": "recut.worlds.entity", "description": "画布上的实体语义操作（方案 A：内容写入统一经画布接口）。op=create 新建实体（需 typeId+name；给 contextId 时自动在该画布层放置投影卡）；op=update 修改一等字段与属性（entityId 必填；attrs 传数组整体替换、缺省保持不变；attrPatch 按 key 合并单条属性，避免为改一个字段回读全量）；op=archive 软删除（归档实体及其子图，可 restore）；op=restore 恢复；op=confirm 把 isProvisional 草稿转正式并产出 revision。archive/restore/confirm 是 Canon 写入，需用户明确要求。", "inputSchema": map[string]any{"type": "object", "required": []string{"worldId", "op"}, "properties": map[string]any{"worldId": map[string]string{"type": "string"}, "op": map[string]any{"type": "string", "enum": []string{"create", "update", "archive", "restore", "confirm"}}, "entityId": map[string]string{"type": "string", "description": "update/archive/restore/confirm 必填。"}, "typeId": map[string]string{"type": "string", "description": "create 必填：预设或自定义 type id。"}, "name": map[string]string{"type": "string"}, "intro": map[string]string{"type": "string"}, "detail": map[string]string{"type": "string"}, "attrs": map[string]any{"type": "array", "description": "有序属性列表；update 时缺省保持不变、传数组整体替换。", "items": map[string]any{"type": "object"}}, "parentId": map[string]string{"type": "string", "description": "可选：父实体 id（递归容器子设定）。"}, "containerRole": map[string]string{"type": "string"}, "isProvisional": map[string]string{"type": "boolean", "description": "create/update 可选：true = 探索草稿，不进 Canon、不产 revision。"}, "contextId": map[string]string{"type": "string", "description": "create 可选：给定时在该画布层自动放置实体投影卡（''=根画布）。"}, "geometry": map[string]any{"type": "object", "description": "create 可选：投影卡几何 {x,y,width,height}。"}, "expectedRevisionId": map[string]string{"type": "string"}}}},
