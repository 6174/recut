...
func worldsMCPToolDefinitions(locale Locale) []map[string]any {
	tools := []map[string]any{
		...
		{"name": "recut.worlds.doc.update", "description": "在一个画布 Document 内应用元素级 ops（insert/update/remove）：kind='entity' 的骨干（refId 指向实体，props 只存视图偏好）或自由元素（text/image/shape/arrow/note/link/media，内容在 props；arrow/link 是语义边：fromElementId 必须指向同文档内的 entity 元素，只有 entity 能作为出发点）。放实体卡只需 {op:'insert', element:{kind:'entity', refKind:'entity', refId}}：服务端自动补 id=`shape:<entityId>`、名称与默认几何。画布视频先经全局提案：调用 recut.video.generate（默认 propose）或 recut.media.propose 落一个 proposed 资产，再把该 assetId 写进 element.kind='media' 的 props.assetId（可带 assetStatus:'generating'）——画布提案的状态与内容都读该资产，确认权只在用户，不得为画布视频直生。返回更新后的 {elements, version}。画布元素永不产 revision。只在用户明确要求摆放画布元素时调用。改已有元素时先用 canvas.doc 读取真实 id。World 节点 id=`shape:world`。" + canvasElementConventions, "inputSchema": canvasDocUpdateSchema()},
			...
			for _, element := range document.Elements {
				...
				if assetID != "" && worlds.media != nil {
					...
					items = append(items, map[string]any{
						...
						"name": element.Name, "status": "pending", "proposal": asset.Metadata["proposal"],
					})
					continue
				}
				// 兼容旧画布：无 assetId、只有 props.proposal 的元素。
