 ...
 * [OUTPUT]: 对外提供全局 recut.worlds.* MCP 工具。读：list/get（单一世界读取入口：概览 + 实体图 +
 * 生产上下文 facts/references/readiness；已合并原 brief 与 readiness）/entities.list/entities.get/entityTypes.list/
 * doc/docs/revisions.list/export/proposals.list 无条件可发现（resolve/readiness/evidence/relations.list
 * 只保留 HTTP 与 App runtime capability，不进 AI 工具面）；
...
func worldsMCPToolDefinitions(locale Locale) []map[string]any {
	tools := []map[string]any{
		...
		{"name": "recut.worlds.get", "description": "读取一个 World 的**单一入口**，一次返回：①概览（身份、统计、当前 revision、world.md `skillMd`、可用实体种类）②**实体图**（`entities` 紧凑：id/typeId/name/intro/media 锚点；`relations`：两端与语义（fromRole/toRole））③**生产上下文**（`facts` 角色/故事/场景/风格字段与 body、`constraints`、`references[]` 可引用项）④**就绪度**（`readiness`：level/score/scenarioId + 按优先级排序的缺失清单 `readiness.missing`）。worldId 必填；可选 `selection`（entityIds/storyId/assetRoles/purpose）聚焦事实、`scenarioId` 选就绪度蓝图，缺省=整库/按类型推荐。大世界 `graphTruncated=true` 时用 `entities.list`（typeId/parentId/text 分页）补实体、用 `entities.get` 读取单个实体的完整关系。非 local 世界只读。", "inputSchema": map[string]any{"type": "object", "required": []string{"worldId"}, "properties": map[string]any{"worldId": map[string]string{"type": "string", "description": "World ID，entityId 只在同一个 worldId 内有效。"}, "revisionId": map[string]string{"type": "string", "description": "可选：缺省为 World 当前 revision。"}, "scenarioId": map[string]any{"type": "string", "enum": []string{"novel-adaptation", "ip-account", "style-system", "brand-guide", "blank"}, "description": "可选：就绪度蓝图；缺省按世界类型推荐。"}, "selection": map[string]any{"type": "object", "properties": map[string]any{"storyId": map[string]string{"type": "string"}, "entityIds": map[string]any{"type": "array", "items": map[string]string{"type": "string"}}, "assetRoles": map[string]any{"type": "array", "items": map[string]string{"type": "string"}}, "purpose": worldPurposeSchema()}}}}},
		...
		{"name": "recut.worlds.create", "description": "创建一个 Creation World。新世界从空开始（不再写入模板空壳实体）；创建后引导用户走 Onboarding（上传素材/粘贴链接/口述），并可用 recut.worlds.get 的 `readiness.missing` 获取工作清单。只在用户明确要求创建 World 时调用；这是 Canon 写入，Agent 不得因推测有帮助而自动创建。", "inputSchema": map[string]any{"type": "object", "required": []string{"name", "type"}, "properties": map[string]any{"name": map[string]string{"type": "string"}, "type": worldKindSchema(), "description": map[string]string{"type": "string"}, "identity": map[string]any{"type": "object"}}}},
		...
		{"name": "recut.worlds.doc.update", "description": "在一个画布 Document 内应用元素级 ops（insert/update/remove）：kind='entity' 的骨干（refId 指向实体，props 只存视图偏好）或自由元素（text/image/shape/arrow/note/link/media，内容在 props；arrow/link 是语义边：fromElementId 必须指向同文档内的 entity 元素，只有 entity 能作为出发点）。放实体卡只需 {op:'insert', element:{kind:'entity', refKind:'entity', refId}}：服务端自动补 id=`shape:<entityId>`、名称与默认几何。画布视频：调用 recut.video.generate 落一个待确认的资产，再把该 assetId 写进 element.kind='media' 的 props.assetId（可带 assetStatus:'generating'）——状态与内容都读该资产，确认权只在用户，不得为画布视频直生。返回更新后的 {elements, version}。画布元素永不产 revision。只在用户明确要求摆放画布元素时调用。改已有元素时先用 canvas.doc 读取真实 id。World 节点 id=`shape:world`。" + canvasElementConventions, "inputSchema": canvasDocUpdateSchema()},
		{"name": "recut.worlds.promote", "description": "把画布草稿提升为正式语义对象并产出 revision：note/text → Entity（可用 typeId 指定 type）；箭头/link 是有语义的边，出发点必须是 entity 元素：entity→entity 变成 world_relations（fromRole，可选 toRole），entity→自由元素变成属性绑定（field 绑定到实体属性，自由元素标记为引用投影并生成 attr 锚点元素，值与右侧属性面板共享 entity.content 单一数据源）。提升后原画布元素保留为投影。这是 Canon 写入，必须显式用户确认。", "inputSchema": map[string]any{"type": "object", "required": []string{"worldId", "elementId"}, "properties": map[string]any{"worldId": map[string]string{"type": "string"}, "elementId": map[string]string{"type": "string"}, "typeId": map[string]string{"type": "string", "description": "可选：便签→实体时的 type id。"}, "fromRole": map[string]string{"type": "string", "description": "可选：箭头→关系时的起点语义 token（缺省 references）。"}, "toRole": map[string]string{"type": "string", "description": "可选：终点语义 token（缺省空=未标记，画布不画双箭头）。"}, "relationType": map[string]string{"type": "string", "description": "已弃用：fromRole 的别名。"}, "field": map[string]string{"type": "string", "description": "可选：箭头→属性绑定时的实体属性 key。"}, "title": map[string]string{"type": "string", "description": "可选：便签→实体时的实体标题。"}, "expectedRevisionId": map[string]string{"type": "string"}}}},
		...
		{"name": "recut.worlds.proposals.list", "description": "列出该 World 画布上的生成提案：画布媒体元素/属性卡引用的 proposed 资产（assetId/elementId/contextId/name/status/proposal）。状态与内容都读全局资产；兼容旧画布上仅带 props.proposal 的元素。确认权只在用户，Agent 落提案后不得代确认；status=generating 时用 jobId 调 recut.job.status 观察。只读。", "inputSchema": map[string]any{"type": "object", "required": []string{"worldId"}, "properties": map[string]any{"worldId": map[string]string{"type": "string"}}}},
	}
	if locale == LocaleEn {
...
var worldsToolDescriptionsEN = map[string]string{
	...
	"recut.worlds.get":              "Read a World's SINGLE entry point. Returns (1) overview: identity, stats, current revision, world.md (skillMd), available entity kinds; (2) the entity graph: entities (compact: id/typeId/name/intro/media anchors) and relations (ends + roles); (3) the production context: facts (characters/stories/locations/styles fields with body), constraints, references[] anchorable items; (4) readiness: level/score/scenarioId plus readiness.missing, the priority-ordered gap list. worldId required; optional selection (entityIds/storyId/assetRoles/purpose) narrows facts and scenarioId picks the readiness blueprint, omitted = the whole world / the type-recommended blueprint. When graphTruncated=true page with entities.list (typeId/parentId/text) and use entities.get for one entity's full fields and relations. Non-local Worlds are read-only.",
	...
	"recut.worlds.create":           "Create a Creation World. New Worlds start empty (no placeholder entities); after creation, guide the user through onboarding (upload material / paste links / dictate) and use recut.worlds.get's `readiness.missing` for the work list. Call only when the user explicitly asks to create a World; this is a Canon write, never create proactively.",
	...
	"recut.worlds.relation":         "Semantic relation ops on the canvas. op=create makes an edge (fromEntityId/toEntityId/fromRole, optional toRole; a canvas arrow + recut.worlds.promote achieves the same); op=update changes the endpoint semantics or direction in place (toRole accepts an empty string to clear it); op=archive soft-deletes (tombstoned, restorable); op=restore restores. A non-empty scopeEntityId makes it a local relation. All but create are Canon writes and need an explicit user request.",
	...
	"recut.worlds.proposals.list":   "List generation proposals on a World's canvas: proposed assets referenced by canvas media elements/attr cards (assetId/elementId/contextId/name/status/proposal). Status and content are read from the global asset; legacy elements carrying only props.proposal remain readable. Confirmation belongs to the user alone (never confirm on the user's behalf after placing a proposal); when status=generating, poll with jobId via recut.job.status. Read-only.",
}
...
func worldsMCPTool(worlds *WorldStore, name string, input map[string]any) (any, error) {
	...
	case "recut.worlds.get":
		// 单一世界读取入口：概览 + 实体图 + 生产上下文（facts/references/missing）。
		selection := WorldSelection{}
		if err = decodeJSONMap(inputMap(input["selection"]), &selection); err != nil {
			return nil, err
		}
		result, err = worlds.GetWorldContext(BriefInput{WorldID: stringValue(input["worldId"]), RevisionID: stringValue(input["revisionId"]), Selection: selection}, stringValue(input["scenarioId"]))
	...
	case "recut.worlds.promote":
		result, err = worlds.PromoteCanvasElement(PromoteCanvasElementInput{
			...
			TypeID: stringValue(input["typeId"]), FromRole: relationFromRoleInput(input), ToRole: stringValue(input["toRole"]),
	...
	case "recut.worlds.proposals.list":
		...
		for _, doc := range docs {
			...
			for _, element := range document.Elements {
				if element.Kind != "media" && element.Kind != "attr" {
					continue
				}
				assetID := stringValue(element.Props["assetId"])
				// 提案真源是全局资产：元素引用 proposed 资产即为一条生成提案。
				if assetID != "" && worlds.media != nil {
					asset, assetErr := worlds.media.GetAsset(assetID)
					if assetErr != nil || asset.Status != AssetStatusProposed {
						continue
					}
					items = append(items, map[string]any{
						"assetId": asset.ID, "elementId": element.ID, "contextId": contextID,
						"name": element.Name, "status": "pending", "generation": asset.Metadata["generation"],
					})
					continue
				}
			}
