...
var mcpToolDescriptions = map[string]map[Locale]string{
	...
	"recut.image.generate": {
		LocaleZh: "**生成必先导演**：提交前先加载 `recut-director`（按任务读相应 references/），形成导演意图——整体讲什么、这个镜头为什么存在、用什么媒介、与相邻画面如何连续——再提交；没有导演意图的生成属于违规，唯一例外是既有导演意图已确定的单点素材替换。提交图片生成，立即返回稳定 assetId（排队/生成中即可引用）。**拿到 assetId 立刻挂到项目/素材/画布并继续做别的事**，不要空等；只有下一步依赖图片内容（读图决策、连续性/质量验收、导出）时才用 jobId 等到终态。用户在素材面板查看结果。世界生图硬规则（未过不提交）：先 `recut.worlds.get({ worldId })` 读 `references[]`，画面会出现主角色时必须带该角色参考图，**参考通过 `imageAssetIds` 传**（image 能力只读该字段，role 绑定不生效）；只有明确无角色的纯空场景可为空。",
		LocaleEn: "**Direct first, then generate**: before submitting, load `recut-director` (read the relevant references/ for the task) and form director intent — what the whole piece is about, why this shot exists, which medium it uses, and how it continues from adjacent frames. Generating without director intent is a violation; the only exception is a single-asset replacement where existing director intent is already settled. Submit an image generation; it immediately returns a stable assetId (referenceable while queued/generating). **Attach the assetId to the project/material/canvas right away and keep working**; only wait on the jobId when the next step actually depends on the image content (image-based decision, continuity/quality acceptance, export). The user reviews the result in the asset panel. World image hard rule (do not submit if unmet): read the world's `references[]` via `recut.worlds.get({ worldId })` first; when the frame may contain the main character, attach that reference, **passed via `imageAssetIds`** (the image capability only reads that field; role bindings do not apply); only a pure empty scene with no character may omit it.",
	},
	"recut.video.generate": {
		LocaleZh: "**生成必先导演**：提交前先加载 `recut-director`（按任务读相应 references/：story / hooks / shot / generation-prompt / b-roll），形成导演意图——整体讲什么、这个镜头为什么存在、用什么媒介、段与段如何连续——再提交；以「段/场景」为单位，连续动作优先一次多镜连续生成。没有导演意图的生成属于违规，唯一例外是既有导演意图已确定的单点素材替换。提交视频生成。视频默认先落为待确认的 proposed 素材（不建任务、不花钱），由用户在素材面板/画布确认后才真正生成；可在「全局设置 → 通用设置 → 生成行为」关闭该门禁，改为提交即生成。**AI 只负责提交与落位，绝不代确认**。始终返回稳定 assetId，**拿到立刻挂到项目/画布并继续**。参考关键帧通过 `imageAssetIds` 传；`aspectRatio` / `durationSec` 显式传入并与画布一致。世界语境硬规则同图片：先读 `references[]`，主线角色必带角色参考。",
		LocaleEn: "**Direct first, then generate**: before submitting, load `recut-director` (read the relevant references/: story / hooks / shot / generation-prompt / b-roll) and form director intent — what the whole piece is about, why this shot exists, which medium it uses, and how segments continue. Work in scene/segment units; a continuous action should prefer a single multi-shot continuous generation. Generating without director intent is a violation; the only exception is a single-asset replacement with settled director intent. Submit a video generation. By default it first lands as a pending proposed asset (no job, no cost) and only runs after the user confirms it in the asset panel/canvas; the gate can be turned off under Settings → General → Generation to generate on submit. **The agent only submits and places it, never confirms.** It always returns a stable assetId — **attach it and continue immediately**. Pass reference keyframes via `imageAssetIds`; pass `aspectRatio` / `durationSec` explicitly and match the canvas. The same world hard rule applies: read `references[]` and attach the main character's reference.",
	},
	...
	"recut.motion-graphic.create": {
		LocaleZh: "创建组件素材的唯一入口。默认直通：传 `source`（单文件 TS/TSX），平台同步构建 + 轻量验证并直接返回 verified（{componentId, versionId, status}），无需子 Agent；有问题再用 motion-graphic.update 迭代。只有复杂组件（需要自主探索/多镜编排/视觉迭代）才传 `author:true` + `items`（每项 brief）走受限作者子 Agent。MG 是全局素材，不绑定任何项目；项目成员关系由消费方（recut.editor）在需要使用时建立。创建本身绝不插入时间线。",
		LocaleEn: "The only entry point to create component assets. Default direct path: pass `source` (single-file TS/TSX); the platform builds and lightly verifies it synchronously and returns verified ({componentId, versionId, status}) with no sub-agent. Iterate with motion-graphic.update if needed. Only complex components (needing autonomous exploration / multi-shot authoring / visual iteration) pass `author:true` plus `items` (each with a brief) to run the restricted author sub-agent. Motion graphics are global assets and never bind a project; the consumer (recut.editor) establishes project membership when it needs to use one. Creation never inserts into the timeline.",
	},
