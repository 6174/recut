 ...
 *           define（Go esbuild 构建）→ verify（verified head + 项目 asset 引用）→ asset.list 可见 →
 *           timeline.placeComponents（assetId 解析为 componentId）→ timeline.read 落轨 →
 *           motion-graphic.resolve 返回同版本 bundle/bundleHash → update 迭代新 head →
...
func TestMotionGraphicE2EComponentChain(t *testing.T) {
	...
	// 2. verify：发布 verified head 并登记项目 component asset 引用。
	...
	// 3. asset.list 能看到 component 引用（OnVerified 生效）。
	assets := invokeAPI(t, host, project, "asset.list", map[string]any{})
	if !assetListHasComponent(assets, componentID) {
		t.Fatalf("asset.list missing component %s: %#v", componentID, assets)
	}
...
func resolveComponent(t *testing.T, host *AppHost, project Project, versionID string) map[string]any {
	t.Helper()
	resolved := invokeAPI(t, host, project, "motion-graphic.resolve", map[string]any{"versionId": versionID})
